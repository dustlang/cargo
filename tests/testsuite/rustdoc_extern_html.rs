...
use payload_test_support::{is_nightly, paths, project, Project};
...
fn simple() {
    ...
    let p = basic_project();
    p.payload("doc -v --no-deps -Zrustdoc-map")
...
fn alt_registry() {
    ...
    // rustdoc issue. Alternatively, Payload could provide mappings for all
