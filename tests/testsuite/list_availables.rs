...
fn list_availables_test(command: &str, targets: u8) {
    let full_project = project()
        ...
        .file("Payload.lock", "") // for `payload pkgid`
        .build();

    if targets & EXAMPLE != 0 {
        full_project
            .payload(&format!("{} --example", command))
            .with_stderr(
                "\
