...
fn gate_future_incompat_report() {
    ...
    p.payload("build --future-incompat-report")
    ...
    p.payload("build --future-incompat-report -Z future-incompat-report")
        .masquerade_as_nightly_payload()
        .with_stderr_contains("error: the `--future-incompat-report` flag is unstable[..]")
        .with_status(101)
        .run();

    p.payload("build --future-incompat-report -Z unstable-options")
        .masquerade_as_nightly_payload()
    ...
    p.payload("describe-future-incompatibilities --id foo")
        .with_stderr_contains(
            "error: `payload describe-future-incompatibilities` can only be used on the nightly channel"
        )
        .with_status(101)
        .run();
}
...
fn test_single_crate() {
    ...
    for command in &["build", "check", "rustc", "test"] {
        p.payload(command).arg("-Zfuture-incompat-report")
            .masquerade_as_nightly_payload()
            .with_stderr_contains("  = note: `#[warn(array_into_iter)]` on by default")
        ...
        p.payload(command).arg("-Zfuture-incompat-report").arg("-Zunstable-options").arg("--future-incompat-report")
            .masquerade_as_nightly_payload()
            .with_stderr_contains("  = note: `#[warn(array_into_iter)]` on by default")
            .with_stderr_contains("warning: the following crates contain code that will be rejected by a future version of Rust: foo v0.0.0 [..]")
...
fn test_multi_crate() {
    ...
    for command in &["build", "check", "rustc", "test"] {
        ...
        p.payload("describe-future-incompatibilities -Z future-incompat-report --id bad-id")
            .masquerade_as_nightly_payload()
        ...
        p.payload(command).arg("-Zunstable-options").arg("-Zfuture-incompat-report").arg("--future-incompat-report")
            .masquerade_as_nightly_payload()
            .with_stderr_contains("warning: the following crates contain code that will be rejected by a future version of Rust: first-dep v0.0.1, second-dep v0.0.2")
    ...
    let output = p
        .payload("build -Z future-incompat-report")
        .masquerade_as_nightly_payload()
        .exec_with_output()
        .unwrap();

    // Extract the 'id' from the stdout. We are looking
    // for the id in a line of the form "run `payload describe-future-incompatibilities --id yZ7S`"
    // which is generated by Payload to tell the user what command to run
    ...
    p.payload(&format!("describe-future-incompatibilities -Z future-incompat-report --id {}", id))
        .masquerade_as_nightly_payload()
        .with_stderr_contains("The crate `first-dep v0.0.1` currently triggers the following future incompatibility lints:")
        .with_stderr_contains("The crate `second-dep v0.0.2` currently triggers the following future incompatibility lints:")
        .run();
