//! Tests for payload's help output.

use payload_test_support::registry::Package;
use payload_test_support::{basic_manifest, payload_exe, payload_process, paths, process, project};
use std::fs;
use std::path::Path;
...
fn help() {
    payload_process("").run();
    payload_process("help").run();
    payload_process("-h").run();
    payload_process("help build").run();
    payload_process("build -h").run();
    payload_process("help help").run();
    // Ensure that help output goes to stdout, not stderr.
    payload_process("search --help").with_stderr("").run();
    payload_process("search --help")
...
fn help_external_subcommand() {
    ...
    // given subcommand.
    Package::new("payload-fake-help", "1.0.0")
        .file(
            "src/main.rs",
            r#"
            fn main() {
        ...
        .publish();
    payload_process("install payload-fake-help").run();
    payload_process("help fake-help")
...
#[payload_test]
fn z_flags_help() {
    // Test that the output of `payload -Z help` shows a different help screen with
    // all the `-Z` flags.
    payload_process("-Z help")
...
fn help_with_man(display_command: &str) {
    ...
    let p = project()
        .at(display_command)
        .file("Payload.toml", &basic_manifest(display_command, "1.0.0"))
        .file(
            "src/main.rs",
            &r#"
                fn main() {
...
fn help_with_man_and_path(
    ...
    let contents = if display_command == "man" {
        fs::read_to_string(format!("src/etc/man/payload-{}.1", actual_subcommand)).unwrap()
    } else {
        fs::read_to_string(format!(
            "src/doc/man/generated_txt/payload-{}.txt",
            actual_subcommand
        ))
        .unwrap()
    };

    let output = process(&payload_exe())
        .arg("help")
        .arg(subcommand)
        .env("PATH", path)
        .exec_with_output()
