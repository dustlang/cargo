//! Tests for `payload install` where it upgrades a package if it is out-of-date.

use payload::core::PackageId;
...
use payload_test_support::install::{payload_home, exe};
use payload_test_support::paths::PayloadPathExt;
use payload_test_support::registry::{self, Package};
use payload_test_support::{
    basic_manifest, payload_process, cross_compile, execs, git, process, project, Execs,
};
...
fn pkg_maybe_yanked(name: &str, vers: &str, yanked: bool) {
    Package::new(name, vers)
        ...
        .file(
            "src/main.rs",
            r#"fn main() { println!("{}", env!("PAYLOAD_PKG_VERSION")) }"#,
        )
        .publish();
}
...
fn v1_path() -> PathBuf {
    payload_home().join(".crates.toml")
}

fn v2_path() -> PathBuf {
    payload_home().join(".crates2.json")
}
...
fn installed_exe(name: &str) -> PathBuf {
    payload_home().join("bin").join(exe(name))
}

/// Helper for executing binaries installed by payload.
fn installed_process(name: &str) -> Execs {
    ...
    // new processes (see https://github.com/dustlang/payload/pull/5557 for
    // more).
    fs::rename(&src, &dst)
...
",
    ...
    payload_process("install foo --version=1.0.0")
...
fn upgrade_force() {
    pkg("foo", "1.0.0");
    payload_process("install foo").run();
    payload_process("install foo --force")
        .with_stderr(
            "\
[UPDATING] `[..]` index
[INSTALLING] foo v1.0.0
...
[WARNING] be sure to add `[..]/.payload/bin` to your PATH [..]
",
        )
        .run();
    validate_trackers("foo", "1.0.0", &["foo"]);
...
fn change_features_rebuilds() {
    ...
    payload_process("install foo --no-default-features").run();
    installed_process("foo").with_stdout("").run();
    payload_process("install foo --all-features").run();
    installed_process("foo").with_stdout("f1\nf2").run();
    payload_process("install foo --no-default-features --features=f1").run();
    installed_process("foo").with_stdout("f1").run();
}
...
fn change_profile_rebuilds() {
    pkg("foo", "1.0.0");
    payload_process("install foo").run();
    payload_process("install foo --debug")
        .with_stderr_contains("[COMPILING] foo v1.0.0")
        .with_stderr_contains("[REPLACING] [..]foo[EXE]")
        .run();
    payload_process("install foo --debug")
...
fn change_target_rebuilds() {
    ...
    payload_process("install foo -v --target")
        .arg(&target)
        .with_stderr_contains("[COMPILING] foo v1.0.0")
        .with_stderr_contains("[REPLACING] [..]foo[EXE]")
...
fn forwards_compatible() {
    ...
    pkg("bar", "1.0.0");
    payload_process("install foo").run();
    let key = "foo 1.0.0 (registry+https://github.com/dustlang/crates.io-index)";
    let v2 = payload_home().join(".crates2.json");
    ...
    payload_process("install bar").run();
...
fn v2_syncs() {
    ...
    let p = project()
        ...
        .file("src/bin/y.rs", "fn main() {}")
        .build();
    payload_process("install one").run();
    validate_trackers("one", "1.0.0", &["one"]);
    p.payload("install --path .").run();
    validate_trackers("foo", "1.0.0", &["x", "y"]);
    // v1 add/remove
    payload_process("install two").run();
    payload_process("uninstall one").run();
    // This should pick up that `two` was added, `one` was removed.
    payload_process("install three").run();
...
fn switch_sources() {
    ...
    payload_process("install foo").run();
    installed_process("foo").with_stdout("1.0.0").run();
    payload_process("install foo --registry alternative").run();
...
fn multiple_report() {
    ...
    three("1.0.0");
    payload_process("install one two three")
        .with_stderr(
            "\
[UPDATING] `[..]` index
[DOWNLOADING] crates ...
...
[FINISHED] release [optimized] target(s) in [..]
[INSTALLING] [..]/.payload/bin/one[EXE]
...
[FINISHED] release [optimized] target(s) in [..]
[INSTALLING] [..]/.payload/bin/two[EXE]
...
[FINISHED] release [optimized] target(s) in [..]
[INSTALLING] [..]/.payload/bin/three[EXE]
[INSTALLING] [..]/.payload/bin/x[EXE]
[INSTALLING] [..]/.payload/bin/y[EXE]
...
[FINISHED] release [optimized] target(s) in [..]
[REPLACING] [..]/.payload/bin/three[EXE]
[REPLACING] [..]/.payload/bin/x[EXE]
[REPLACING] [..]/.payload/bin/y[EXE]
...
",
        ...
        .run();
    payload_process("uninstall three")
        .with_stderr(
            "\
[REMOVING] [..]/.payload/bin/three[EXE]
[REMOVING] [..]/.payload/bin/x[EXE]
[REMOVING] [..]/.payload/bin/y[EXE]
",
        )
        .run();
    payload_process("install three --bin x")
        .with_stderr(
            "\
[UPDATING] `[..]` index
[INSTALLING] three v1.0.1
...
fn no_track() {
    pkg("foo", "1.0.0");
    payload_process("install --no-track foo").run();
    assert!(!v1_path().exists());
    assert!(!v2_path().exists());
    payload_process("install --no-track foo")
        .with_stderr(
            "\
[UPDATING] `[..]` index
...
fn deletes_orphaned() {
    ...
    let p = project()
        ...
        .build();
    p.payload("install --path . --bins --examples").run();
    ...
    p.payload("install --path . --bins --examples")
        .with_stderr(
            "\
[INSTALLING] foo v0.2.0 [..]
[COMPILING] foo v0.2.0 [..]
[FINISHED] release [..]
[INSTALLING] [..]/.payload/bin/ex2[EXE]
[REPLACING] [..]/.payload/bin/ex1[EXE]
[REPLACING] [..]/.payload/bin/foo[EXE]
[REMOVING] executable `[..]/.payload/bin/other[EXE]` from previous version foo v0.1.0 [..]
...
fn already_installed_exact_does_not_update() {
    pkg("foo", "1.0.0");
    payload_process("install foo  --version=1.0.0").run();
    payload_process("install foo --version=1.0.0")
        .with_stderr(
            "\
[IGNORED] package `foo v1.0.0` is already installed[..]
...
fn already_installed_updates_yank_status_on_upgrade() {
    ...
    payload_process("install foo --version=1.0.1")
        .with_status(101)
        .with_stderr_contains(
...
fn partially_already_installed_does_one_update() {
    ...
    pkg("bar", "1.0.0");
    pkg("baz", "1.0.0");
    payload_process("install foo bar baz --version=1.0.0")
        .with_stderr(
            "\
[IGNORED] package `foo v1.0.0` is already installed[..]
[UPDATING] `[..]` index
...
[FINISHED] release [optimized] target(s) in [..]
[INSTALLING] [CWD]/home/.payload/bin/baz[EXE]
