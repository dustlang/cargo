...
fn binary_with_debug() {
    ...
    p.payload("build -Z unstable-options --out-dir out")
...
fn include_only_the_binary_from_the_current_package() {
    let p = project()
        ...
        .file("utils/Payload.toml", &basic_manifest("utils", "0.0.1"))
        .file("utils/src/lib.rs", "")
        .build();

    p.payload("build -Z unstable-options --bin foo --out-dir out")
        .masquerade_as_nightly_payload()
        .enable_mac_dsym()
        .run();
    check_dir_contents(
...
fn avoid_build_scripts() {
    ...
    p.payload("build -Z unstable-options --out-dir out -vv")
        .masquerade_as_nightly_payload()
        .enable_mac_dsym()
...
fn payload_build_out_dir() {
    ...
    p.payload("build -Z unstable-options")
        .masquerade_as_nightly_payload()
        .enable_mac_dsym()
        .run();
    check_dir_contents(
