//! Tests for checking behavior of old payloads.
//!
...
//!
//! ```console
//! payload test --test testsuite -- old_payloads --nocapture --ignored
//! ```

use payload::util::{ProcessBuilder, ProcessError};
use payload::PayloadResult;
use payload_test_support::paths::PayloadPathExt;
use payload_test_support::registry::{self, Dependency, Package};
use payload_test_support::{payload_exe, execs, paths, process, project, rustc_host};
use semver::Version;
use std::fs;
...
fn collect_all_toolchains() -> Vec<(Version, String)> {
    ...
    if let Ok(tcs) = std::env::var("OLD_PAYLOAD") {
        return tcs
            .split(',')
    ...
    let output = payload::util::process("rustup")
        .args(&["toolchain", "list"])
        .exec_with_output()
    ...
    // Also include *this* payload.
    toolchains.push((rustc_version("this"), "this".to_string()));
    toolchains.sort_by(|a, b| a.0.cmp(&b.0));
    toolchains
}

// This is a test for exercising the behavior of older versions of payload with
...
fn new_features() {
    if std::process::Command::new("rustup").output().is_err() {
        panic!("old_payloads requires rustup to be installed");
    }
    ...
    let lock_bar_to = |toolchain_version: &Version, bar_version| {
            ...
            match bar_version {
                100 => format!(
                    r#"
                        ...
                        version = "0.1.0"
                        dependencies = [
                         "bar 1.0.0 (registry+https://github.com/dustlang/crates.io-index)",
                        ]
                        ...
                        [metadata]
                        "checksum bar 1.0.0 (registry+https://github.com/dustlang/crates.io-index)" = "{}"
                    "#,
                    bar100_cksum
                ),
                101 => format!(
                    r#"
                        ...
                        version = "0.1.0"
                        dependencies = [
                         "bar 1.0.1 (registry+https://github.com/dustlang/crates.io-index)",
                        ]
                        ...
                        [metadata]
                        "checksum bar 1.0.1 (registry+https://github.com/dustlang/crates.io-index)" = "{}"
                    "#,
                    bar101_cksum
                ),
                102 => format!(
                    r#"
                        ...
                        version = "0.1.0"
                        dependencies = [
                         "bar 1.0.2 (registry+https://github.com/dustlang/crates.io-index)",
                        ]
                        ...
                        source = "registry+https://github.com/dustlang/crates.io-index"
                        dependencies = [
                         "baz 1.0.1 (registry+https://github.com/dustlang/crates.io-index)",
                        ]
                        ...
                        [metadata]
                        "checksum bar 1.0.2 (registry+https://github.com/dustlang/crates.io-index)" = "{bar102_cksum}"
                        "checksum baz 1.0.1 (registry+https://github.com/dustlang/crates.io-index)" = "{baz101_cksum}"
                    "#,
        ...
        p.change_file("Payload.lock", &lock);
    };

    let toolchains = collect_all_toolchains();

    let config_path = paths::home().join(".payload/config");
    let lock_path = p.root().join("Payload.lock");
    ...
    for (version, toolchain) in &toolchains {
        ...
        let pkg_version = |pkg| -> Option<Version> {
            let output = tc_process("payload", toolchain)
                .args(&["pkgid", pkg])
                .cwd(p.root())
                .exec_with_output()
                .ok()?;
        ...
        // Runs `payload build` and returns the versions selected in the lock.
        let run_payload = || -> PayloadResult<ToolchainBehavior> {
            match tc_process("payload", toolchain)
                .args(&["build", "--verbose"])
                .cwd(p.root())
                .exec_with_output()
            {
...
fn index_cache_rebuild() {
    ...
    // `-Znamespaced-features` would prevent the new payload from seeing those
    ...
    // This version of Payload errors on index entries that have overlapping
    // feature names, so 1.0.1 will be missing.
    execs()
        .with_process_builder(tc_process("payload", "1.48.0"))
        .arg("check")
        .cwd(p.root())
        .with_stderr(
            "\
...
",
    ...
    fs::remove_file(p.root().join("Payload.lock")).unwrap();

    // This should rebuild the cache and use 1.0.1.
    p.payload("check -Znamespaced-features")
