//! Tests for the `payload generate-lockfile` command.

use payload_test_support::registry::Package;
use payload_test_support::{basic_manifest, paths, project, ProjectBuilder};
use std::fs;
...
fn no_index_update() {
    ...
    p.payload("generate-lockfile -Zno-index-update")
...
"#;
    ...
    p.change_file("Payload.lock", &data);
...
fn preserve_line_endings_issue_2076() {
    ...
    let lockfile = p.root().join("Payload.lock");
    p.payload("generate-lockfile").run();
    ...
    assert!(lock0.starts_with("# This file is automatically @generated by Payload.\n# It is not intended for manual editing.\n"));

    let lock1 = lock0.replace("\n", "\r\n");
    p.change_file("Payload.lock", &lock1);

    p.payload("generate-lockfile").run();

    let lock2 = p.read_lockfile();

    assert!(lock2.starts_with("# This file is automatically @generated by Payload.\r\n# It is not intended for manual editing.\r\n"));
    assert_eq!(lock1, lock2);
}

#[payload_test]
fn payload_update_generate_lockfile() {
    let p = project().file("src/main.rs", "fn main() {}").build();

    let lockfile = p.root().join("Payload.lock");
    assert!(!lockfile.is_file());
    p.payload("update").with_stdout("").run();
    assert!(lockfile.is_file());
