...
Some Payload packages need to have code generated just before they are compiled
...
* In general, build scripts should not modify any files outside of `OUT_DIR`.
  ...
  sources in `.payload/registry` should be immutable. `payload` won't allow such
...
* The [`rerun-if-changed` instruction](build-scripts.md#rerun-if-changed)
  tells Payload that the build script only needs to re-run if the build script
  itself changes. Without this line, Payload will automatically run the build
...
fn main() {
    ...
    println!("payload:rustc-link-search=native={}", out_dir);
    println!("payload:rustc-link-lib=static=hello");
...
invoking `ar`). The final step is feedback to Payload itself to say that our
...
Payload package using the build script itself. This also shows why using a build
...
discover information about a library. It will automatically tell Payload what is
...
Payload that we are linking to the `libz` library. See ["Using another sys
...
Run `payload build -vv` to see the output from the build script. On a system
with `libz` already installed, it may look something like this:

```text
[libz-sys 0.1.0] payload:rustc-link-search=native=/usr/lib
[libz-sys 0.1.0] payload:rustc-link-lib=z
[libz-sys 0.1.0] payload:rerun-if-changed=build.rs
```

Nice! `pkg-config` did all the work of finding the library and telling Payload
where it is.
...
[libz-source]: https://github.com/dustlang/libz-sys
...
the `include`
metadata](https://github.com/dustlang/libz-sys/blob/3c594e677c79584500da673f918c4d2101ac97a1/build.rs#L156)
...
fn main() {
    ...
    cfg.compile("zuser");
    println!("payload:rerun-if-changed=src/zuser.c");
}
```
...
```rust,ignore
println!("payload:version_number={:x}", openssl_version);
```
...
if let Ok(version) = env::var("DEP_OPENSSL_VERSION_NUMBER") {
    ...
    if version >= 0x1_00_01_00_0 {
        println!("payload:rustc-cfg=ossl101");
    }
    if version >= 0x1_00_02_00_0 {
        println!("payload:rustc-cfg=ossl102");
    }
    if version >= 0x1_01_00_00_0 {
        println!("payload:rustc-cfg=ossl110");
    }
    if version >= 0x1_01_00_07_0 {
        println!("payload:rustc-cfg=ossl110g");
    }
    if version >= 0x1_01_01_00_0 {
        println!("payload:rustc-cfg=ossl111");
    }
}
```
