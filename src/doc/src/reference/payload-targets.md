...
Payload packages consist of *targets* which correspond to source files which can
...
`Payload.toml` manifest, often [inferred automatically](#target-auto-discovery)
...
table in `Payload.toml`.

```toml
# Example of customizing the library in Payload.toml.
[lib]
crate-type = ["cdylib"]
bench = false
```
...
in the `[[bin]]` tables in `Payload.toml`.

Binaries can use the public API of the package's library. They are also linked
with the [`[dependencies]`][dependencies] defined in `Payload.toml`.

You can run individual binaries with the [`payload run`] command with the `--bin
<bin-name>` option. [`payload install`] can be used to copy the executable to a
common location.

```toml
# Example of customizing binaries in Payload.toml.
[[bin]]
name = "cool-tool"
test = false
bench = false
...
[`[dev-dependencies]`][dev-dependencies] defined in `Payload.toml`.
...
You can run individual executable examples with the [`payload run`] command with
the `--example <example-name>` option. Library examples can be built with
[`payload build`] with the `--example <example-name>` option. [`payload install`]
with the `--example <example-name>` option can be used to copy executable
binaries to a common location. Examples are compiled by [`payload test`] by
default to protect them from bit-rotting. Set [the `test`
field](#the-test-field) to `true` if you have `#[test]` functions in the
example that you want to run with [`payload test`].

### Tests

There are two styles of tests within a Payload project:
...
Tests are run with the [`payload test`] command. By default, Payload and `rustc`
...
tests. When you run [`payload test`], Payload will compile each of these files as
...
Each integration test results in a separate executable binary, and [`payload
...
[`payload test`] to only run the tests within that module.

Binary targets are automatically built if there is an integration test. This
allows an integration test to execute the binary to exercise and test its
behavior. The `PAYLOAD_BIN_EXE_<name>` [environment variable] is set when the
integration test is built so that it can use the [`env` macro] to locate the
executable.

[environment variable]: environment-variables.md#environment-variables-payload-sets-for-crates
[`env` macro]: ../../std/macro.env.html

### Benchmarks

Benchmarks provide a way to test the performance of your code using the
[`payload bench`] command. They follow the same structure as [tests](#tests),
...
sections in `Payload.toml` support similar configuration for specifying how a
...
[`payload test`]. The default is `true` for lib, bins, and tests.

> **Note**: Examples are built by [`payload test`] by default to ensure they
...
tested by default by [`payload test`]. This is only relevant for libraries, it
...
default by [`payload bench`]. The default is `true` for lib, bins, and
benchmarks.

#### The `doc` field

The `doc` field indicates whether or not the target is included in the
documentation generated by [`payload doc`] by default. The default is `true` for
...
By default, Payload automatically determines the targets to build based on the
...
`bin`, this would present a problem because Payload would usually attempt to
...
To prevent Payload from inferring `src/bin/mod.rs` as an executable, set
`autobins = false` in `Payload.toml` to disable auto-discovery:
...
> is `false` if at least one target is manually defined in `Payload.toml`.
