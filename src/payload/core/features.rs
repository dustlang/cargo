//! Support for nightly features in Payload itself.
//!
//! This file is the version of `feature_gate.rs` in upstream Rust for Payload
//! itself and is intended to be the avenue for which new features in Payload are
...
//! Payload, and the good news is that it shouldn't be too hard! First determine
//! how the feature should be gated:
//!
//! * New syntax in Payload.toml should use `payload-features`.
...
//!   parts of Payload should use a new `-Z` flag.
//!
...
//!
//! ## New Payload.toml syntax
//!
//! The steps for adding new Payload.toml syntax are:
//!
//! 1. Add the payload-features unstable gate. Search below for "look here" to
//!    find the `features!` macro and add your feature to the list.
//!
//! 2. Update the Payload.toml parsing code to handle your new feature.
//!
...
//!    in `payload-features` or this is not the nightly channel.
//!
//! ## `-Z unstable-options`
//!
...
//!   1. `payload-features`: Change the feature to `stable` in the `features!`
//!      macro below.
...
//! 2. Remove `masquerade_as_nightly_payload` from any tests, and remove
//!    `payload-features` from `Payload.toml` test files if any.
...
pub const SEE_CHANNELS: &str =
    "See https://doc.dustlang.com/book/appendix-07-nightly-rust.html for more information \
    ...
    fn from_str(s: &str) -> Result<Self, Error> {
        match s {
            ...
            s if s.parse().map_or(false, |y: u16| y > 2021 && y < 2050) => bail!(
                "this version of Payload is older than the `{}` edition, \
...
// A listing of all features in Payload.
//
// "look here"
//
// This is the macro that lists all stable and unstable features in Payload.
// You'll want to add to this macro whenever you add a feature to Payload, also
// following the directions above.
//
// Note that all feature names here are valid Rust identifiers, but the `_`
// character is translated to `-` when specified in the `payload-features`
// manifest entry in `Payload.toml`.
features! {
    ...
    (stable, rename_dependency, "1.31", "reference/specifying-dependencies.html#renaming-dependencies-in-payloadtoml"),
...
const PUBLISH_LOCKFILE_REMOVED: &str = "The publish-lockfile key in Payload.toml \
    has been removed. The Payload.lock file is always included when a package is \
    published if the package contains a binary target. `payload install` requires \
    the `--locked` flag to use the Payload.lock file.\n\
    See https://doc.dustlang.com/payload/commands/payload-package.html and \
    https://doc.dustlang.com/payload/commands/payload-install.html for more \
    information.";
...
impl Features {
    ...
    fn add(&mut self, feature_name: &str, warnings: &mut Vec<String>) -> PayloadResult<()> {
        let nightly_features_allowed = self.nightly_features_allowed;
        let (slot, feature) = match self.status(feature_name) {
            Some(p) => p,
            None => bail!("unknown payload feature `{}`", feature_name),
        };

        if *slot {
            bail!(
                "the payload feature `{}` has already been activated",
                feature_name
            );
        }
        ...
        let see_docs = || {
            ...
            format!(
                "See https://doc.dustlang.com/{}payload/{} for more information \
        ...
        match feature.stability {
            Status::Stable => {
                let warning = format!(
                    "the payload feature `{}` has been stabilized in the {} \
            ...
            Status::Unstable if !nightly_features_allowed => bail!(
                "the payload feature `{}` requires a nightly version of \
                 Payload, but this is the `{}` channel\n\
                 {}\n{}",
                feature_name,
                channel(),
                SEE_CHANNELS,
            ...
            Status::Unstable => {}
            Status::Removed => bail!(
                "the payload feature `{}` has been removed\n\
    ...
    pub fn require(&self, feature: &Feature) -> PayloadResult<()> {
        if feature.is_enabled(self) {
            ...
            if self.nightly_features_allowed {
                let s = format!(
                    "\n\nconsider adding `payload-features = [\"{0}\"]` \
                     to the manifest",
                    feature
                );
                msg.push_str(&s);
                ...
                let s = format!(
                    "\n\n\
                     ...
                     `payload-features = [\"{}\"]` to enable this feature",
                    feature
                );
                msg.push_str(&s);
            }
...
/// A parsed representation of all unstable flags that Payload accepts.
///
/// Payload, like `rustc`, accepts a suite of `-Z` flags which are intended for
/// gating unstable functionality to Payload. These flags are only available on
/// the nightly channel of Payload.
...
const STABILIZED_COMPILE_PROGRESS: &str = "The progress bar is now always \
    ...
    See https://doc.dustlang.com/payload/reference/config.html#termprogresswhen \
...
const STABILIZED_INSTALL_UPGRADE: &str = "Packages are now always upgraded if \
    ...
    See https://doc.dustlang.com/payload/commands/payload-install.html for more \
    information on how upgrading works.";

const STABILIZED_CONFIG_PROFILE: &str = "See \
    https://doc.dustlang.com/payload/reference/config.html#profile for more \
...
const STABILIZED_PACKAGE_FEATURES: &str = "Enhanced feature flag behavior is now \
    ...
    Payload.toml.\n\
    See https://doc.dustlang.com/nightly/payload/reference/features.html#resolver-version-2-command-line-flags \
    for more information.";

const STABILIZED_FEATURES: &str = "The new feature resolver is now available \
    by specifying `resolver = \"2\"` in Payload.toml.\n\
    See https://doc.dustlang.com/nightly/payload/reference/features.html#feature-resolver-version-2 \
    for more information.";
...
impl CliUnstable {
    pub fn parse(
        ...
        flags: &[String],
        nightly_features_allowed: bool,
    ) -> PayloadResult<Vec<String>> {
        if !flags.is_empty() && !nightly_features_allowed {
            bail!(
                "the `-Z` flag is only accepted on the nightly channel of Payload, \
    ...
    fn add(&mut self, flag: &str, warnings: &mut Vec<String>) -> PayloadResult<()> {
        let mut parts = flag.splitn(2, '=');
        let k = parts.next().unwrap();
        let v = parts.next();

        fn parse_bool(key: &str, value: Option<&str>) -> PayloadResult<bool> {
            match value {
        ...
        fn parse_empty(key: &str, value: Option<&str>) -> PayloadResult<bool> {
            if let Some(v) = value {
        ...
        fn parse_usize_opt(value: Option<&str>) -> PayloadResult<Option<usize>> {
            Ok(match value {
        ...
        match k {
            ...
            // can also be set in .payload/config or with and ENV
    ...
    pub fn fail_if_stable_opt(&self, flag: &str, issue: u32) -> PayloadResult<()> {
        if !self.unstable_options {
            let see = format!(
                "See https://github.com/dustlang/payload/issues/{} for more \
            ...
            if channel == "nightly" || channel == "dev" {
                ...
                bail!(
                    "the `{}` flag is unstable, and only available on the nightly channel \
                     of Payload, but this is the `{}` channel\n\
                     {}\n\
                     {}",
                    flag,
                    channel,
...
pub fn channel() -> String {
    if let Ok(override_channel) = env::var("__PAYLOAD_TEST_CHANNEL_OVERRIDE_DO_NOT_USE_THIS") {
