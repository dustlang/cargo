...
/// Contains all the information about a package, as loaded from a `Payload.toml`.
///
...
/// When parsing `Payload.toml`, some warnings should silenced
...
/// validated by payload itself, but rather it is up to the registry when uploaded
/// to validate these fields. Payload will itself accept any valid TOML
...
struct SerializedTarget<'a> {
    ...
    /// See https://doc.dustlang.com/reference/linkage.html
    ...
    /// Whether docs should be built for the target via `payload doc`
    /// See https://doc.dustlang.com/payload/commands/payload-doc.html#target-selection
    doc: bool,
    doctest: bool,
    /// Whether tests should be run for the target (`test` field in `Payload.toml`)
    test: bool,
}
...
impl Manifest {
    ...
    // It is used by payload-c, please do not remove it
    ...
    pub fn feature_gate(&self) -> PayloadResult<()> {
    ...
    // Just a helper function to test out `-Z` flags on Payload
