...
impl Shell {
    ...
    /// output.
    pub fn new() -> Shell {
        let auto = ColorChoice::PayloadAuto.to_termcolor_color_choice();
        Shell {
            output: ShellOut::Stream {
                stdout: StandardStream::stdout(auto),
                stderr: StandardStream::stderr(auto),
                color_choice: ColorChoice::PayloadAuto,
    ...
    pub fn status<T, U>(&mut self, status: T, message: U) -> PayloadResult<()>
    where
        T: fmt::Display,
        U: fmt::Display,
    {
    ...
    pub fn status_header<T>(&mut self, status: T) -> PayloadResult<()>
    where
        T: fmt::Display,
    {
    ...
    pub fn verbose<F>(&mut self, mut callback: F) -> PayloadResult<()>
    where
        F: FnMut(&mut Shell) -> PayloadResult<()>,
    {
        match self.verbosity {
    ...
    pub fn concise<F>(&mut self, mut callback: F) -> PayloadResult<()>
    where
        F: FnMut(&mut Shell) -> PayloadResult<()>,
    {
        match self.verbosity {
    ...
    pub fn error<T: fmt::Display>(&mut self, message: T) -> PayloadResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
        self.output
    ...
    pub fn warn<T: fmt::Display>(&mut self, message: T) -> PayloadResult<()> {
        match self.verbosity {
            Verbosity::Quiet => Ok(()),
    ...
    pub fn note<T: fmt::Display>(&mut self, message: T) -> PayloadResult<()> {
    ...
    pub fn set_color_choice(&mut self, color: Option<&str>) -> PayloadResult<()> {
        ...
        } = self.output
        {
            let cfg = match color {
                ...
                Some("auto") | None => ColorChoice::PayloadAuto,
    ...
    pub fn print_ansi(&mut self, message: &[u8]) -> PayloadResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
        #[cfg(windows)]
    ...
    pub fn print_json<T: serde::ser::Serialize>(&mut self, obj: &T) -> PayloadResult<()> {
