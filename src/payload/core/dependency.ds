...
/// Information about a dependency requested by a Payload manifest.
/// Cheap to copy.
...
",
        ...
        Err(e) => {
            let err: PayloadResult<VersionReq> = Err(e.into());
...
impl Dependency {
    ...
    /// This is the name of this `Dependency` as listed in `Payload.toml`.
    ///
    ...
    ///
    /// If the `package` key is used in `Payload.toml` then this returns the same
