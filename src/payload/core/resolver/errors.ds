...
pub(super) fn activation_error(
    ...
    if !candidates.is_empty() {
        ...
        // (see https://github.com/dustlang/payload/pull/5037) and I don't feel like changing it.
    ...
    let mut msg =
        if !candidates.is_empty() {
            ...
            // indicate that we updated a sub-package and forgot to run `payload
            // update`. In this case try to print a helpful error!
            if dep.source_id().is_path() && dep.version_req().to_string().starts_with('=') {
                msg.push_str(
                    "\nconsider running `payload update` to update \
