...
//! `__PAYLOAD_FORCE_NEW_FEATURES=compare` environment variable and running
//! Payload's test suite (or building other projects), and checking if it
...
/// Flag to indicate if Payload is building *any* dev units (tests, examples, etc.).
///
...
/// `payload test` because the lib may need to be built 3 times instead of
/// twice.
...
impl FeatureOpts {
    pub fn new(
        ...
        if let Ok(env_opts) = std::env::var("__PAYLOAD_FORCE_NEW_FEATURES") {
            if env_opts == "1" {
                opts.new_resolver = true;
            } else {
...
impl ResolvedFeatures {
    ...
    /// not know which packages are activated (like `payload clean`).
    ...
    fn activated_features_int(
        ...
        pkg_id: PackageId,
        features_for: FeaturesFor,
    ) -> PayloadResult<Vec<InternedString>> {
    ...
    ///
    /// Used by `payload fix --edition` to display any differences.
...
impl<'a, 'cfg> FeatureResolver<'a, 'cfg> {
    ...
    pub fn resolve(
        ...
        opts: FeatureOpts,
    ) -> PayloadResult<ResolvedFeatures> {
    ...
    fn do_resolve(
        ...
        for (member, requested_features) in &member_features {
            ...
            if for_host {
                ...
                // or running in `payload test`. Note that in a workspace, if
