...
//!   dependency in Payload (e.g., when you write `foo = "0.1.2"`) is
...
//! ## Performance
//!
//! Note that this is a relatively performance-critical portion of Payload. The
...
impl RemainingCandidates {
    ...
    fn next(
        ...
        for b in self.remaining.by_ref() {
            ...
            // semver. Payload dictates that you can't duplicate multiple
...
/// `conflicting_activations`.
///
/// Read <https://github.com/dustlang/payload/pull/4834>
...
fn check_cycles(resolve: &Resolve) -> PayloadResult<()> {
...
fn check_duplicate_pkgs_in_lockfile(resolve: &Resolve) -> PayloadResult<()> {
