...
impl ResolverProgress {
    pub fn new() -> ResolverProgress {
        ResolverProgress {
            ...
            #[cfg(debug_assertions)]
            slow_cpu_multiplier: std::env::var("PAYLOAD_TEST_SLOW_CPU_MULTIPLIER")
                .ok()
                .and_then(|m| m.parse().ok())
                .unwrap_or(1),
        }
    }
    pub fn shell_status(&mut self, config: Option<&Config>) -> PayloadResult<()> {
...
impl ResolveBehavior {
    pub fn from_manifest(resolver: &str) -> PayloadResult<ResolveBehavior> {
        match resolver {
...
pub struct ResolveOpts {
    ...
    ///
    /// This may be set to `false` by things like `payload install` or `-Z avoid-dev-deps`.
    pub dev_deps: bool,
