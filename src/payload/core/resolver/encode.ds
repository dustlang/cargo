//! Definition of how to encode a `Resolve` into a TOML `Payload.lock` file
//!
//! This module contains all machinery necessary to parse a `Resolve` from a
//! `Payload.lock` as well as serialize a `Resolve` to a `Payload.lock`.
//!
//! ## Changing `Payload.lock`
//!
//! In general Payload is quite conservative about changing the format of
//! `Payload.lock`. Usage of new features in Payload can change `Payload.lock` at any
//! time, but otherwise changing the serialization of `Payload.lock` is a
//! difficult operation to do that we typically avoid.
//!
//! The main problem with changing the format of `Payload.lock` is that it can
//! cause quite a bad experience for end users who use different versions of
//! Payload. If every PR to a project oscillates between the stable channel's
//! encoding of Payload.lock and the nightly channel's encoding then that's a
//! pretty bad experience.
//!
//! We do, however, want to change `Payload.lock` over time. (and we have!). To do
//! this the rules that we currently have are:
//!
//! * Add support for the new format to Payload. This involves code changes in
//!   Payload itself, likely by adding a new variant of `ResolveVersion` and
...
//!     not be used yet.
//!
//!   * Preserve the new format if found. This means that if Payload finds the new
...
//!   format.
//!
//! This migration scheme in general means that Payload we'll get *support* for a
//! new format into Payload ASAP, but it won't be exercised yet (except in Payload's
...
//! it.
//!
//! While this does mean that projects' `Payload.lock` changes over time, it's
//! typically a pretty minimal effort change that's just "check in what's
//! there".
//!
//! ## Historical changes to `Payload.lock`
//!
//! Listed from most recent to oldest, these are some of the changes we've made
//! to `Payload.lock`'s serialization format:
//!
//! * A `version` marker is now at the top of the lock file which is a way for
//!   super-old Payloads (at least since this was implemented) to give a formal
//!   error if they see a lock file from a super-future Payload. Additionally as
...
//!   merge conflicts being generated on `Payload.lock`. Updating a version of a
...
//!   in the `[metadata]` table of `Payload.lock` which all versions of Payload
//!   since 1.0 have preserved. The goal of this was to start recording
//!   checksums so mirror sources can be verified.
//!
//! ## Other oddities about `Payload.lock`
//!
//! There's a few other miscellaneous weird things about `Payload.lock` that you
...
//!   unique names.
//!
//! * The `[metadata]` table in `Payload.lock` is intended to be a generic mapping
//!   of strings to strings that's simply preserved by Payload. This was a very
//!   early effort to be forward compatible against changes to `Payload.lock`'s
...
//!   `src/payload/ops/lockfile.rs` which basically renders a `toml::Value` in a
...
impl EncodableResolve {
    /// Convert a `Payload.lock` to a Resolve.
    ///
    ...
    pub fn into_resolve(self, original: &str, ws: &Workspace<'_>) -> PayloadResult<Resolve> {
        ...
        let mut version = match self.version {
            ...
            Some(n) => bail!(
                "lock file version `{}` was found, but this version of Payload \
                 does not understand this lock file, perhaps Payload needs \
                 to be updated?",
                n,
            ),
            // Historically Payload did not have a version indicator in lock
        ...
        let mut lookup_id = |enc_id: &EncodablePackageId| -> Option<PackageId> {
            ...
            match &enc_id.source {
                ...
                None => {
                    ...
                    // `Payload.lock` or something like that, so just ignore this.
                    } else {
                        None
                    }
                }
        ...
        // conditions.
        //
        // Payload is trying to write new lockfies in the "v2 format" but if you
        // have no dependencies, for example, then the lockfile encoded won't
        // really have any indicator that it's in the new format (no
        // dependencies or checksums listed). This means that if you type `payload
        // new` followed by `payload build` it will generate a "v2 format" lock
        // file since none previously existed. When reading this on the next
        // `payload build`, however, it generates a new lock file because when
...
fn build_path_deps(ws: &Workspace<'_>) -> PayloadResult<HashMap<String, SourceId>> {
    // If a crate is **not** a path source, then we're probably in a situation
    // such as `payload install` with a lock file from a remote dependency. In
...
impl FromStr for EncodablePackageId {
    ...
    fn from_str(s: &str) -> PayloadResult<EncodablePackageId> {
