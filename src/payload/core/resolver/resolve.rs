...
pub struct Resolve {
    ...
    /// from `Payload.toml`. We need a `HashSet` here because the same package
    ...
    /// found in the `[metadata]` section of `Payload.lock`, preserved for
    /// forwards compatibility.
    metadata: Metadata,
    /// `[patch]` entries that did not match anything, preserved in
    /// `Payload.lock` as the `[[patch.unused]]` table array. Tracking unused
    /// patches helps prevent Payload from being forced to re-update the
    ...
    /// Version of the `Payload.lock` format, see
    /// `payload::core::resolver::encode` for more.
    version: ResolveVersion,
    summaries: HashMap<PackageId, Summary>,
}

/// A version to indicate how a `Payload.lock` should be serialized. Currently
...
/// to the `Payload.lock` format, but we've yet to see how that strategy pans out.
...
impl Resolve {
    ...
    pub fn merge_from(&mut self, previous: &Resolve) -> PayloadResult<()> {
...
",
                ...
                // that future Payload figured out how to checksum something or
...
",
    ...
    pub fn query(&self, spec: &str) -> PayloadResult<PackageId> {
        PackageIdSpec::query_str(spec, self.iter())
    }

    pub fn specs_to_ids(&self, specs: &[PackageIdSpec]) -> PayloadResult<Vec<PackageId>> {
...
impl Default for ResolveVersion {
    /// The default way to encode new or updated `Payload.lock` files.
    ///
    ...
    /// those generated by `payload update` (update everything) or building after
    /// a `payload new` (where no lock file previously existed). This is also used
    /// for *updated* lock files such as when a dependency is added or when a
    /// version requirement changes. In this situation Payload's updating the lock
