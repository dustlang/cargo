...
use crate::util::errors::{PayloadResult, PayloadResultExt, ManifestError};
use crate::util::interning::InternedString;
use crate::util::paths;
use crate::util::toml::{read_manifest, TomlDependency, TomlProfiles};
use crate::util::{config::ConfigRelativePath, Config, Filesystem, IntoUrl};

/// The core abstraction in Payload for working with a workspace of crates.
///
...
pub struct Workspace<'cfg> {
    ...
    // This path is a path to where the current payload subcommand was invoked
    // from. That is the `--manifest-path` argument to Payload, and
    ...
    // missing, `package.workspace` is missing, and no `Payload.toml` above
    ...
    // `payload install` or `payload package` commands.
    is_ephemeral: bool,
    ...
    // needed by the current configuration (such as in payload install). In some
    ...
    // If `true`, then the resolver will ignore any existing `Payload.lock`
    // file. This is set for `payload install` without `--locked`.
    ignore_lock: bool,
...
impl<'cfg> Workspace<'cfg> {
    ...
    pub fn new(manifest_path: &Path, config: &'cfg Config) -> PayloadResult<Workspace<'cfg>> {
    ...
    pub fn new_virtual(
        ...
        config: &'cfg Config,
    ) -> PayloadResult<Workspace<'cfg>> {
        let mut ws = Workspace::new_default(current_manifest, config);
        ws.root_manifest = Some(root_path.join("Payload.toml"));
        ws.target_dir = config.target_dir()?;
        ws.packages
            .packages
    ...
    ///
    /// This is currently only used in niche situations like `payload install` or
    /// `payload package`.
    pub fn ephemeral(
        package: Package,
        config: &'cfg Config,
    ...
    /// actually a "virtual Payload.toml", in which case an error is returned
    /// indicating that something else should be passed.
    pub fn current(&self) -> PayloadResult<&Package> {
    ...
    pub fn current_mut(&mut self) -> PayloadResult<&mut Package> {
    ...
    /// `Payload.toml` which is the root of this workspace.
    pub fn root(&self) -> &Path {
        self.root_manifest().parent().unwrap()
    }

    /// Returns the path of the `Payload.toml` which is the root of this
    /// workspace.
    ...
    fn config_patch(&self) -> PayloadResult<HashMap<Url, Vec<Dependency>>> {
        ...
        if config_patch.is_some() && !self.config.cli_unstable().patch_in_config {
            self.config.shell().warn("`[patch]` in payload config was ignored, the -Zpatch-in-config command-line flag is required".to_owned())?;
        ...
        for message in warnings {
            self.config
                .shell()
                .warn(format!("[patch] in payload config: {}", message))?
        }
    ...
    ///
    /// This may be from a virtual crate or an actual crate.
    pub fn root_patch(&self) -> PayloadResult<HashMap<Url, Vec<Dependency>>> {
    ...
    pub fn load_workspace_config(&mut self) -> PayloadResult<Option<WorkspaceRootConfig>> {
    ...
    /// at `manifest_path`.
    ///
    /// This will parse the `Payload.toml` at `manifest_path` and then interpret
    ...
    fn find_root(&mut self, manifest_path: &Path) -> PayloadResult<Option<PathBuf>> {
        ...
        for path in paths::ancestors(manifest_path, None).skip(2) {
            ...
            let ances_manifest_path = path.join("Payload.toml");
            ...
            // Don't walk across `PAYLOAD_HOME` when we're looking for the
            // workspace root. Sometimes a package will be organized with
            // `PAYLOAD_HOME` pointing inside of the workspace root or in the
    ...
    /// the workspace.
    fn find_members(&mut self) -> PayloadResult<()> {
        ...
        for path in &members_paths {
            self.find_path_deps(&path.join("Payload.toml"), &root_manifest_path, false)?;
        }

        if let Some(default) = default_members_paths {
            for path in default {
                let normalized_path = paths::normalize_path(&path);
                let manifest_path = normalized_path.join("Payload.toml");
                if !self.members.contains(&manifest_path) {
                    ...
                    // include `/Payload.toml`, and because excluded paths may not
                    // be crates.
    ...
    /// Returns the unstable nightly-only features enabled via `payload-features` in the manifest.
    ...
    fn validate(&mut self) -> PayloadResult<()> {
    ...
    fn validate_unique_names(&self) -> PayloadResult<()> {
    ...
    fn validate_workspace_roots(&self) -> PayloadResult<()> {
    ...
    fn validate_members(&mut self) -> PayloadResult<()> {
    ...
    fn error_if_manifest_not_in_members(&mut self) -> PayloadResult<()> {
    ...
    fn validate_manifest(&mut self) -> PayloadResult<()> {
        if let Some(ref root_manifest) = self.root_manifest {
                ...
                .filter(|p| p.manifest_path() != root_manifest)
            {
                ...
                let emit_warning = |what| -> PayloadResult<()> {
                    let msg = format!(
    ...
    pub fn load(&self, manifest_path: &Path) -> PayloadResult<Package> {
    ...
    pub fn preload(&self, registry: &mut PackageRegistry<'cfg>) {
        ...
        // `payload install`. Things like git repositories will actually have a
        // `PathSource` with multiple entries in it, so the logic below is
        // mostly just an optimization for normal `payload build` in workspaces
        // during development.
    ...
    /// `true`. This is used for generating `Payload.lock`, which must include
    /// all members with all features enabled.
    pub fn members_with_features(
        &self,
        specs: &[PackageIdSpec],
        requested_features: &RequestedFeatures,
    ) -> PayloadResult<Vec<(&Package, RequestedFeatures)>> {
        assert!(
    ...
    fn members_with_features_new(
        ...
        let members: Vec<(&Package, RequestedFeatures)> = self
            ...
            .collect();
        if members.is_empty() {
            // `payload build -p foo`, where `foo` is not a member.
...
impl<'cfg> Packages<'cfg> {
    ...
    fn load(&mut self, manifest_path: &Path) -> PayloadResult<&MaybePackage> {
...
impl WorkspaceRootConfig {
    ...
    fn members_paths(&self, globs: &[String]) -> PayloadResult<Vec<PathBuf>> {
    ...
    fn expand_member_path(path: &Path) -> PayloadResult<Vec<PathBuf>> {
