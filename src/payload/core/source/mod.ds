...
use crate::util::{PayloadResult, Config};
...
pub trait Source {
    ...
    fn query_vec(&mut self, dep: &Dependency) -> PayloadResult<Vec<Summary>> {
        let mut ret = Vec::new();
    ...
    fn download_now(self: Box<Self>, package: PackageId, config: &Config) -> PayloadResult<Package>
    where
    ...
    fn finish_download(&mut self, package: PackageId, contents: Vec<u8>) -> PayloadResult<Package>;
    ...
    fn verify(&self, _pkg: PackageId) -> PayloadResult<()> {
        Ok(())
    }
...
impl<'a, T: Source + ?Sized + 'a> Source for Box<T> {
    ...
    fn finish_download(&mut self, id: PackageId, data: Vec<u8>) -> PayloadResult<Package> {
    ...
    fn verify(&self, pkg: PackageId) -> PayloadResult<()> {
        (**self).verify(pkg)
    }
