...
use crate::util::{CanonicalUrl, PayloadResult, Config, IntoUrl};
use log::trace;
use serde::de;
use serde::ser;
...
impl SourceId {
    ...
    fn new(kind: SourceKind, url: Url) -> PayloadResult<SourceId> {
    ...
    ///
    /// ```
    /// use payload::core::SourceId;
    /// SourceId::from_url("git+https://github.com/alexcrichton/\
    ///                     libssh2-static-sys#80e71a3021618eb05\
    ///                     656c58fb7c5ef5f12bc747f");
    /// ```
    pub fn from_url(string: &str) -> PayloadResult<SourceId> {
    ...
    pub fn for_path(path: &Path) -> PayloadResult<SourceId> {
        let url = path.into_url()?;
        SourceId::new(SourceKind::Path, url)
    }

    /// Creates a `SourceId` from a Git reference.
    pub fn for_git(url: &Url, reference: GitReference) -> PayloadResult<SourceId> {
        SourceId::new(SourceKind::Git(reference), url.clone())
    }

    /// Creates a SourceId from a registry URL.
    pub fn for_registry(url: &Url) -> PayloadResult<SourceId> {
        SourceId::new(SourceKind::Registry, url.clone())
    }

    /// Creates a SourceId from a local registry path.
    pub fn for_local_registry(path: &Path) -> PayloadResult<SourceId> {
        let url = path.into_url()?;
        SourceId::new(SourceKind::LocalRegistry, url)
    }

    /// Creates a `SourceId` from a directory path.
    pub fn for_directory(path: &Path) -> PayloadResult<SourceId> {
        let url = path.into_url()?;
    ...
    ///
    /// This is the main payload registry by default, but it can be overridden in
    /// a `.payload/config`.
    pub fn crates_io(config: &Config) -> PayloadResult<SourceId> {
    ...
    pub fn alt_registry(config: &Config, key: &str) -> PayloadResult<SourceId> {
    ...
    ) -> PayloadResult<Box<dyn super::Source + 'a>> {
...
// The hash of SourceId is used in the name of some Payload folders, so shouldn't
