...
struct UnitTime {
    unit: Unit,
    /// A string describing the payload target.
    target: String,
...
impl<'cfg> Timings<'cfg> {
    ...
    fn report_html(
        ...
        let filename = format!("payload-timing-{}.html", timestamp);
        ...
        paths::link_or_copy(&filename, "payload-timing.html")?;
        self.config
            .shell()
...
"#,
    ...
    fn write_js_data(&self, f: &mut impl Write) -> PayloadResult<()> {
    ...
    fn write_unit_table(&self, f: &mut impl Write) -> PayloadResult<()> {
        write!(
            f,
            r#"
...
<html>
<head>
  <title>Payload Build Timings — {ROOTS}</title>
  <meta charset="utf-8">
<style type="text/css">
html {
