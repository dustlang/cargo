...
use payload_platform::CfgExpr;
use semver::Version;

use super::BuildContext;
use crate::core::compiler::{CompileKind, Metadata, Unit};
use crate::core::Package;
use crate::util::{self, config, join_paths, process, PayloadResult, Config, ProcessBuilder};
...
impl<'cfg> Compilation<'cfg> {
    pub fn new<'a>(bcx: &BuildContext<'a, 'cfg>) -> PayloadResult<Compilation<'cfg>> {
    ...
    /// Returns a [`ProcessBuilder`] appropriate for running a process for the
    /// target platform. This is typically used for `payload run` and `payload
    /// test`.
    ///
    ...
    fn fill_env(
        ...
        if cfg!(target_os = "macos") && dylib_path_is_empty {
            ...
            // set or set to an empty string. Since Payload is explicitly setting
        ...
        let payload_exe = self.config.payload_exe()?;
        cmd.env(crate::PAYLOAD_ENV, payload_exe);
        ...
        cmd.env("PAYLOAD_MANIFEST_DIR", pkg.root())
            .env("PAYLOAD_PKG_VERSION_MAJOR", &pkg.version().major.to_string())
            .env("PAYLOAD_PKG_VERSION_MINOR", &pkg.version().minor.to_string())
            .env("PAYLOAD_PKG_VERSION_PATCH", &pkg.version().patch.to_string())
            .env(
                "PAYLOAD_PKG_VERSION_PRE",
                &pre_version_component(pkg.version()),
            )
            .env("PAYLOAD_PKG_VERSION", &pkg.version().to_string())
            .env("PAYLOAD_PKG_NAME", &*pkg.name())
            .env(
                "PAYLOAD_PKG_DESCRIPTION",
                metadata.description.as_ref().unwrap_or(&String::new()),
            )
            .env(
                "PAYLOAD_PKG_HOMEPAGE",
                metadata.homepage.as_ref().unwrap_or(&String::new()),
            )
            .env(
                "PAYLOAD_PKG_REPOSITORY",
            ...
            .env(
                "PAYLOAD_PKG_LICENSE_FILE",
                metadata.license_file.as_ref().unwrap_or(&String::new()),
            )
            .env("PAYLOAD_PKG_AUTHORS", &pkg.authors().join(":"))
            .cwd(pkg.root());
...
fn fill_rustc_tool_env(mut cmd: ProcessBuilder, unit: &Unit) -> ProcessBuilder {
    if unit.target.is_bin() {
        cmd.env("PAYLOAD_BIN_NAME", unit.target.name());
    }
    cmd.env("PAYLOAD_CRATE_NAME", unit.target.crate_name());
    cmd
}
...
fn target_runner(
    ...
    kind: CompileKind,
) -> PayloadResult<Option<(PathBuf, Vec<String>)>> {
    ...
    if let Some((key, runner)) = cfgs.next() {
        anyhow::bail!(
            "several matching instances of `target.'cfg(..)'.runner` in `.payload/config`\n\
