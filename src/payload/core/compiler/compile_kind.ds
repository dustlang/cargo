    ...
    pub fn from_requested_targets(
        ...
        if !targets.is_empty() {
            return Ok(targets
                ...
                // more than once...
                .collect::<PayloadResult<BTreeSet<_>>>()?
...
/// Abstraction for the representation of a compilation target that Payload has.
///
...
/// here in Payload. Note that for JSON target files this `CompileTarget` stores a
/// full canonicalized path to the target.
///
/// The main reason for this existence is to handle JSON target files where when
/// we call rustc we pass full paths but when we use it for Payload's purposes
...
impl CompileTarget {
    pub fn new(name: &str) -> PayloadResult<CompileTarget> {
        let name = name.trim();
        if name.is_empty() {
    ...
    /// Payload for configuration and such.
    ///
