...
//!     # Payload-specific directories added in the future should use some prefix
//!     # like `.` to avoid name collisions.
//!     debug/  # or release/
//!
//!         # File used to lock the directory to prevent multiple payload processes
...
//!     doc/
//!
//!     # Used by `payload package` and `payload publish` to build a `.crate` file.
//!     package/
//!
...
use crate::util::{PayloadResult, FileLock};
...
pub struct Layout {
    ...
    doc: PathBuf,
    /// The lockfile for a build (`.payload-lock`). Will be unlocked when this
    /// struct is `drop`ped.
    _lock: FileLock,
}
...
impl Layout {
    ...
    pub fn new(
        ...
        // compile.
        let lock = dest.open_rw(".payload-lock", ws.config(), "build directory")?;
