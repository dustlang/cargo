...
pub struct Work {
    inner: Box<dyn FnOnce(&JobState<'_>) -> PayloadResult<()> + Send>,
}

impl Work {
    pub fn new<F>(f: F) -> Work
    where
        F: FnOnce(&JobState<'_>) -> PayloadResult<()> + Send + 'static,
    {
    ...
    pub fn call(self, tx: &JobState<'_>) -> PayloadResult<()> {
        (self.inner)(tx)
    }
...
impl Job {
    ...
    /// computation.
    pub fn run(self, state: &JobState<'_>) -> PayloadResult<()> {
