...
use crate::util::{PayloadResult, Config, RustfixDiagnosticServer};
use anyhow::bail;
use serde::ser;
...
pub struct BuildConfig {
    ...
    /// Force Payload to do a full rebuild and treat each target as changed.
    ...
    /// A thread used by `payload fix` to receive messages on a socket regarding
...
impl BuildConfig {
    ...
    pub fn new(
        ...
        if jobs.is_some() && config.jobserver_from_env().is_some() {
            config.shell().warn(
                "a `-j` argument was passed to Payload but Payload is \
    ...
    pub fn single_requested_kind(&self) -> PayloadResult<CompileKind> {
...
pub enum MessageFormat {
    Human,
    Json {
        /// Whether rustc diagnostics are rendered by payload or included into the
        /// output stream.
