...
pub fn parse_unstable_flag(value: Option<&str>) -> Vec<String> {
    ...
    // declare dependencies in Payload.toml.
...
pub fn resolve_std<'cfg>(
    ...
    crates: &[String],
) -> PayloadResult<(PackageSet<'cfg>, Resolve, ResolvedFeatures)> {
    ...
    let current_manifest = src_path.join("library/test/Payload.toml");
...
fn detect_sysroot_src_path(target_data: &RustcTargetData) -> PayloadResult<PathBuf> {
    if let Some(s) = env::var_os("__PAYLOAD_TESTS_ONLY_SRC_ROOT") {
        return Ok(s.into());
    }
    ...
    let src_path = target_data
        ...
        .join("src")
        .join("rust");
    let lock = src_path.join("Payload.lock");
    if !lock.exists() {
        anyhow::bail!(
