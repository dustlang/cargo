...
use crate::util::{paths, PayloadResult, PayloadResultExt, ProcessBuilder, Rustc};
use payload_platform::{Cfg, CfgExpr};
...
pub struct TargetInfo {
    ...
    /// `Some((prefix, suffix))`, for example `libpayload.so` would be
...
impl FileType {
    ...
    /// The filename for this FileType that Payload should use when "uplifting"
...
impl TargetInfo {
    ...
    fn not_user_specific_cfg(cfg: &PayloadResult<Cfg>) -> bool {
    ...
    fn file_types(
        ...
        flavor: FileFlavor,
        target_triple: &str,
    ) -> PayloadResult<Option<Vec<FileType>>> {
        ...
        if target_triple.starts_with("wasm32-") && crate_type == CrateType::Bin && suffix == ".js" {
            ...
            ret.push(FileType {
                ...
                // `foo_bar.wasm`. Payload will translate the underscore and
    ...
    fn discover_crate_type(&self, crate_type: &CrateType) -> PayloadResult<Option<(String, String)>> {
    ...
    pub fn rustc_outputs(
        ...
        target_triple: &str,
    ) -> PayloadResult<(Vec<FileType>, Vec<CrateType>)> {
        match mode {
...
/// Rust library like `libpayload.rlib`, we have prefix "lib" and suffix "rlib").
///
...
) -> PayloadResult<Option<(String, String)>> {
...
///
///  - `target.*.rustflags` from the config (.payload/config)
...
fn env_args(
    ...
    // architecture. Payload's present architecture makes it quite
...
impl RustcTargetData {
    ...
    /// configuration in Payload or presenting to users.
...
impl RustDocFingerprint {
    ...
    fn read(cx: &Context<'_, '_>) -> PayloadResult<Self> {
    ...
    fn write<'a, 'cfg>(&self, cx: &Context<'a, 'cfg>) -> PayloadResult<()> {
        paths::write(
    ...
    fn remove_doc_dirs(doc_dirs: &[&Path]) -> PayloadResult<()> {
        doc_dirs
            .iter()
            .filter(|path| path.exists())
            .map(|path| paths::remove_dir_all(&path))
            .collect::<PayloadResult<()>>()
    }

    /// This function checks whether the latest version of `Rustc` used to compile this
    /// `Workspace`'s docs was the same as the one is currently being used in this `payload doc`
    /// call.
    ///
    ...
    /// any versioning.
    pub fn check_rustdoc_fingerprint(cx: &Context<'_, '_>) -> PayloadResult<()> {
