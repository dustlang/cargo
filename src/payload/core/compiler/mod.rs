...
use crate::util::errors::{self, PayloadResult, PayloadResultExt, ProcessError, VerboseError};
...
pub trait Executor: Send + Sync + 'static {
    ...
    /// In case of an `Err`, Payload will not continue with the build process for
    /// this package.
    fn exec(
        &self,
        cmd: &ProcessBuilder,
        ...
        target: &Target,
        mode: CompileMode,
        on_stdout_line: &mut dyn FnMut(&str) -> PayloadResult<()>,
        on_stderr_line: &mut dyn FnMut(&str) -> PayloadResult<()>,
    ) -> PayloadResult<()>;
...
/// A `DefaultExecutor` calls rustc without doing anything else. It is Payload's
/// default behaviour.
#[derive(Copy, Clone)]
...
fn rustc(cx: &mut Context<'_, '_>, unit: &Unit, exec: &Arc<dyn Executor>) -> PayloadResult<Work> {
    ...
    return Ok(Work::new(move |state| {
        ...
        if rustc_dep_info_loc.exists() {
            ...
            // This mtime shift allows Payload to detect if a source file was
...
fn link_targets(cx: &mut Context<'_, '_>, unit: &Unit, fresh: bool) -> PayloadResult<Work> {
    let bcx = cx.bcx;
    let outputs = cx.outputs(unit)?;
    ...
    Ok(Work::new(move |state| {
        ...
        // above. This means that `payload build` will produce binaries in
        // `target/debug` which one probably expects.
        let mut destinations = vec![];
        for output in outputs.iter() {
            let src = &output.path;
            // This may have been a `payload rustc` command which changes the
...
fn prepare_rustc(
    ...
    if is_primary {
        base.env("PAYLOAD_PRIMARY_PACKAGE", "1");
    }
...
fn rustdoc(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<Work> {
    let bcx = cx.bcx;
...
///
/// Payload always uses JSON output. This has several benefits, such as being
...
/// which Payload will extract and display to the user.
fn add_error_format_and_color(cx: &Context<'_, '_>, cmd: &mut ProcessBuilder, pipelined: bool) {
    ...
    if pipelined {
        ...
        // to emit a message that payload will intercept.
...
fn build_base_args(
    ...
    if unit.mode.is_check() {
        ...
        // future Payload session as part of a pipelined compile.
    ...
    if test && unit.target.harness() {
        ...
        // Payload has historically never compiled `--test` binaries with
        ...
        // passed when compiling the test. We require, in Payload, an unstable
    ...
    // Add `PAYLOAD_BIN_` environment variables for building tests.
    if unit.target.is_test() || unit.target.is_bench() {
            ...
            .filter(|target| target.is_bin())
        {
            ...
            let key = format!("PAYLOAD_BIN_EXE_{}", bin_target.name());
            ...
            .find(|dep| !dep.unit.mode.is_doc() && dep.unit.target.is_lib())
        {
            bcx.config.shell().warn(format!(
                "The package `{}` \
                 ...
                 Payload.toml. This warning might turn into a hard error in the future.",
...
) -> PayloadResult<Vec<OsString>> {
    let mut result = Vec::new();
    let deps = cx.unit_deps(unit);

    // Closure to add one dependency to `result`.
    let mut link_to =
        |dep: &UnitDep, extern_crate_name: InternedString, noprelude: bool| -> PayloadResult<()> {
...
struct OutputOptions {
    /// What format we're emitting from Payload itself.
...
fn on_stderr_line_inner(
    ...
    // Depending on what we're emitting from Payload itself, we figure out what to
    // do with this JSON message.
    match options.format {
        // In the "human" output formats (human/short) or if diagnostic messages
        // from rustc aren't being included in the output of Payload's JSON
        ...
        // enabled. Payload always asks for ANSI colors from rustc. This allows
    ...
    //
    // In these cases look for a matching directive and inform Payload internally
    ...
    // from the compiler, so wrap it in an external Payload JSON message
    ...
    // instead. We want the stdout of Payload to always be machine parseable as
