...
//!
//! Rust code is typically organized as a set of Payload packages. The
...
fn calc_deps_of_std(
    ...
    std_roots: &HashMap<CompileKind, Vec<Unit>>,
) -> PayloadResult<Option<UnitGraph>> {
...
fn deps_of_roots(roots: &[Unit], mut state: &mut State<'_, '_>) -> PayloadResult<()> {
    for unit in roots.iter() {
        // Dependencies of tests/benches should not have `panic` set.
        // We check the global test mode to see if we are running in `payload
...
fn deps_of(unit: &Unit, state: &mut State<'_, '_>, unit_for: UnitFor) -> PayloadResult<()> {
        ...
        && (unit.target.is_test() || unit.target.is_bench())
    {
        ret.extend(
            unit.pkg
                ...
                .collect::<PayloadResult<Vec<UnitDep>>>()?,
        );
    }
...
fn dep_build_script(
    ...
    unit.pkg
        ...
        .map(|t| {
            ...
            // PAYLOAD_FEATURE_* environment variables to the features as a
            // normal dep.
            //
            // If `is_for_host_features` here is `true`, that means that this
            // package is being used as a build dependency or proc-macro, and
            // so we only want to set PAYLOAD_FEATURE_* variables for the host
            // side of the graph.
            //
            ...
            // features instead of just checking `PAYLOAD_FEATURE_*` at runtime.
