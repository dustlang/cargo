...
use crate::util::{self, PayloadResult, StableHasher};

/// This is a generic version number that can be changed to make
/// backwards-incompatible changes to any file structures in the output
/// directory. For example, the fingerprint files or the build-script
/// output files. Normally payload updates ship with rustc updates which will
/// cause a new hash due to the rustc version changing, but this allows
/// payload to be extra careful to deal with different versions of payload that
...
///
/// This also acts as the main layer of caching provided by Payload.
/// For example, we want to cache `payload build` and `payload doc` separately, so that running one
...
impl<'a, 'cfg: 'a> CompilationFiles<'a, 'cfg> {
    ...
    ) -> PayloadResult<Arc<Vec<OutputFile>>> {
        self.outputs[unit]
    ...
    fn uplift_to(&self, unit: &Unit, file_type: &FileType, from_path: &Path) -> Option<PathBuf> {
        ...
        //   latest copies (even if the dylib was built from a previous payload
        ...
        //   historically how Payload has operated. This is primarily useful to
    ...
    fn calc_outputs(
        ...
        let ret = match unit.mode {
            ...
            CompileMode::Doctest => {
                ...
                // but Payload does not know about that.
                vec![]
            }
            CompileMode::Test
            | CompileMode::Build
...
fn compute_metadata(
    ...
    // Seed the contents of `__PAYLOAD_DEFAULT_LIB_METADATA` to the hasher if present.
    // This should be the release channel, to get a different hash for each channel.
    if let Ok(ref channel) = env::var("__PAYLOAD_DEFAULT_LIB_METADATA") {
...
fn should_use_metadata(bcx: &BuildContext<'_, '_>, unit: &Unit) -> bool {
    ...
    //   See https://github.com/dustlang/rust/issues/72550#issuecomment-638501691
    //   for more details.
    //
    // This is only done for local packages, as we don't expect to export
    // dependencies.
    //
    // The __PAYLOAD_DEFAULT_LIB_METADATA env var is used to override this to
    ...
    // See https://github.com/dustlang/payload/issues/3005
    ...
    if (unit.target.is_dylib()
        ...
        && env::var("__PAYLOAD_DEFAULT_LIB_METADATA").is_err()
    {
        return false;
    }
    true
