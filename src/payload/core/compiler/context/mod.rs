...
impl<'a, 'cfg> Context<'a, 'cfg> {
    pub fn new(bcx: &'a BuildContext<'a, 'cfg>) -> PayloadResult<Self> {
    ...
    pub fn compile(mut self, exec: &Arc<dyn Executor>) -> PayloadResult<Compilation<'cfg>> {
        ...
        // any versioning (See https://github.com/dustlang/payload/issues/8461).
    ...
    pub fn get_executable(&mut self, unit: &Unit) -> PayloadResult<Option<PathBuf>> {
        for output in self.outputs(unit)?.iter() {
    ...
    pub fn prepare_units(&mut self) -> PayloadResult<()> {
    ...
    /// place.
    pub fn prepare(&mut self) -> PayloadResult<()> {
    ...
    pub fn outputs(&self, unit: &Unit) -> PayloadResult<Arc<Vec<OutputFile>>> {
    ...
    /// Returns the list of filenames read by payload to generate the `BuildContext`
    /// (all `Payload.toml`, etc.).
    pub fn build_plan_inputs(&self) -> PayloadResult<Vec<PathBuf>> {
    ...
    fn check_collisions(&self) -> PayloadResult<()> {
        ...
        let suggestion =
            "Consider changing their names to be unique or compiling them separately.\n\
             ...
             <https://github.com/dustlang/payload/issues/6313>.";
         ...
         -> PayloadResult<()> {
            if unit.target.name() == other_unit.target.name() {
                ...
                self.bcx.config.shell().warn(format!(
                    ...
                    {}\n\
                    If this looks unexpected, it may be a bug in Payload. Please file a bug report at\n\
                    https://github.com/dustlang/payload/issues/ with as much information as you\n\
                    can provide.\n\
                    {} running on `{}` target `{}`\n\
                    First unit: {:?}\n\
                    Second unit: {:?}",
        ...
        for unit in keys {
            for output in self.outputs(unit)?.iter() {
                if let Some(other_unit) = output_collisions.insert(output.path.clone(), unit) {
                    if unit.mode.is_doc() {
                        // See https://github.com/dustlang/rust/issues/56169
                        // and https://github.com/dustlang/rust/issues/61378
    ...
    pub fn new_jobserver(&mut self) -> PayloadResult<Client> {
