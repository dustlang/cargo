...
const PAYLOAD_WARNING: &str = "payload:warning=";
...
pub struct BuildScripts {
    ...
    ///
    /// Payload will use this `to_link` vector to add `-L` flags to compiles as we
...
pub fn prepare(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<Job> {
...
fn build_work(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<Job> {
    ...
    cmd.env("OUT_DIR", &script_out_dir)
        .env("PAYLOAD_MANIFEST_DIR", unit.pkg.root())
    ...
    if let Some(links) = unit.pkg.manifest().links() {
        cmd.env("PAYLOAD_MANIFEST_LINKS", links);
    }

    // Be sure to pass along all enabled features for this package, this is the
    // last piece of statically known information that we have.
    for feat in &unit.features {
        cmd.env(&format!("PAYLOAD_FEATURE_{}", super::envify(feat)), "1");
    }
    ...
    for (k, v) in cfg_map {
        if k == "debug_assertions" {
            ...
            // That is because Payload queries rustc without any profile settings.
            continue;
        }
        let k = format!("PAYLOAD_CFG_{}", super::envify(&k));
        match v {
            Some(list) => {
                cmd.env(&k, list.join(","));
            }
    ...
    let dirty = Work::new(move |state| {
        ...
        let output = cmd
            .exec_with_streaming(
                &mut |stdout| {
                    if let Some(warning) = stdout.strip_prefix(PAYLOAD_WARNING) {
...
impl BuildOutput {
    ...
    pub fn parse(
        ...
        for line in input.split(|b| *b == b'\n') {
            ...
            if iter.next() != Some("payload") {
                // skip this line since it doesn't start with "payload:"
                continue;
            }
            ...
            let (key, value) = match (key, value) {
                ...
                // Line started with `payload:` but didn't match `key=value`.
            ...
            match key {
                ...
                "rustc-link-arg-bins" => {
                    if extra_link_arg {
                        ...
                        warnings.push(format!("payload:{} requires -Zextra-link-arg flag", key));
                    }
                }
                "rustc-link-arg" => {
                    if extra_link_arg {
                ...
                "rustc-env" => {
                    ...
                    // See https://github.com/dustlang/payload/issues/7088.
                    if key == "RUSTC_BOOTSTRAP" {
                        // If RUSTC_BOOTSTRAP is already set, the user of Payload knows about
                        ...
                        if nightly_features_allowed {
                            ...
                            anyhow::bail!("Cannot set `RUSTC_BOOTSTRAP={}` from {}.\n\
                                note: Crates cannot set `RUSTC_BOOTSTRAP` themselves, as doing so would subvert the stability guarantees of Rust for your project.\n\
                                help: If you're sure you want to do this in your project, set the environment variable `RUSTC_BOOTSTRAP={}` before running payload instead.",
                                val,
                                whence,
                                pkg_name,
                            );
    ...
    pub fn parse_rustc_flags(
        value: &str,
        whence: &str,
    ) -> PayloadResult<(Vec<PathBuf>, Vec<String>)> {
    ...
    pub fn parse_rustc_env(value: &str, whence: &str) -> PayloadResult<(String, String)> {
...
fn prepare_metabuild(cx: &Context<'_, '_>, unit: &Unit, deps: &[String]) -> PayloadResult<()> {
...
pub fn build_map(cx: &mut Context<'_, '_>) -> PayloadResult<()> {
