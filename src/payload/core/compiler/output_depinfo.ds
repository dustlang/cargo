...
//!
//! ```makefile
//! /path/to/target/debug/deps/payload-b6219d178925203d: src/bin/main.rs src/bin/payload/cli.rs # … etc.
//! ```
//!
...
//!
//! On top of all this, Payload emits its own dep-info files in the output
//! directory. This is done for every "uplifted" artifact. These are intended
//! to be used with external build systems so that they can detect if Payload
...
//! be detected via changes to `Payload.lock`.
...
use crate::util::{internal, PayloadResult};

fn render_filename<P: AsRef<Path>>(path: P, basedir: Option<&str>) -> PayloadResult<String> {
...
pub fn output_depinfo(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<()> {
    let bcx = cx.bcx;
