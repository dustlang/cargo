//! # Fingerprints
//!
//! This module implements change-tracking so that Payload can know whether or
//! not something needs to be recompiled. A Payload `Unit` can be either "dirty"
...
//! __PAYLOAD_DEFAULT_LIB_METADATA[^4]           |             | ✓
//! package_id                                 |             | ✓
...
//!
//! [^4]: `__PAYLOAD_DEFAULT_LIB_METADATA` is set by rustbuild to embed the
...
//!   `PAYLOAD_LOG=payload::core::compiler::fingerprint=trace payload build` can be
//!   used to display this log information.
//! - A "dep-info" file which is a translation of rustc's `*.d` dep-info files
//!   to a Payload-specific format that tweaks file names and is optimized for
//!   reading quickly.
...
//! ### dep-info files
//!
//! Payload passes the `--emit=dep-info` flag to `rustc` so that `rustc` will
...
//! the crate. This file is used by Payload to know which files to check to see
//! if the crate will need to be rebuilt.
//!
//! After `rustc` exits successfully, Payload will read the dep info file and
...
//! more detail). Note that Payload parses the special `# env-var:...` comments in
//! dep-info files to learn about environment variables that the rustc compile
//! depends on. Payload then later uses this to trigger a recompile if a
//! referenced env var changes (even if the source didn't change).
//!
//! There is also a third dep-info file. Payload will extend the file created by
...
//! primarily to support rustc development, so that Payload can check the
//! implicit dependency to the standard library (which lives in the sysroot).
//! We want Payload to recompile whenever the standard library rlib/dylibs
...
//! running. Payload can't know whether or not the file was included in the
...
//!
//! Rustdoc does not emit a dep-info file, so Payload currently has a relatively
...
//! this mode, Payload will use `LocalFingerprint::Precalculated`. See the
...
//!   a project directory Payload will continue to preserve all build artifacts
...
//! test suite nowadays and lord knows Payload gets enough testing in the wild!
//!
//! ## Build scripts
//!
...
//! output directory in a file called `output`. Payload parses this file when
...
//! used, the Package ID will change, causing a rebuild). Payload currently
...
//! number of problems, particularly with Payload's testsuite which does rapid
...
//! <https://github.com/dustlang/payload/issues?q=is%3Aissue+is%3Aopen+label%3AA-rebuild-detection>
...
pub fn prepare_target(cx: &mut Context<'_, '_>, unit: &Unit, force: bool) -> PayloadResult<Job> {
    ...
    // fingerprint at step 3 ensures that Payload never mistakes a partially
...
/// `DependencyQueue`, but it also needs to be retained here because Payload can
...
pub struct Fingerprint {
    ...
    /// `payload rustc` or `payload rustdoc`.
    profile: u64,
...
enum LocalFingerprint {
    ...
    /// that's overall a mistake and causes bugs in Payload. We shouldn't use this
    /// for build scripts.
    Precalculated(String),

    /// This is used for crate compilations. The `dep_info` file is a relative
    /// path anchored at `target_root(...)` to the dep-info file that Payload
...
impl LocalFingerprint {
    ...
    fn find_stale_item(
        ...
        pkg_root: &Path,
        target_root: &Path,
    ) -> PayloadResult<Option<StaleItem>> {
        match self {
...
impl Fingerprint {
    ...
    fn compare(&self, old: &Fingerprint) -> PayloadResult<()> {
...
impl DepFingerprint {
    fn new(cx: &mut Context<'_, '_>, parent: &Unit, dep: &UnitDep) -> PayloadResult<DepFingerprint> {
...
impl StaleItem {
    ...
    /// conjunction with `PAYLOAD_LOG` to determine why Payload is recompiling
...
///
/// This fingerprint is used by Payload to learn about when information such as:
///
...
/// dependencies.
fn calculate(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<Arc<Fingerprint>> {
...
fn calculate_normal(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<Fingerprint> {
...
fn calculate_run_custom_build(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<Fingerprint> {
...
/// overridden via `.payload/config`. The closure is much more complicated.
///
...
/// `&BuildDeps`. This is the parsed version of a build script, and when Payload
...
fn build_script_local_fingerprints(
    ...
    Box<
        dyn FnOnce(
                &BuildDeps,
                Option<&dyn Fn() -> PayloadResult<String>>,
            ) -> PayloadResult<Option<Vec<LocalFingerprint>>>
            + Send,
    >,
    bool,
) {
    ...
    if let Some(fingerprint) = build_script_override_fingerprint(cx, unit) {
        ...
        return (
            Box::new(
                move |_: &BuildDeps, _: Option<&dyn Fn() -> PayloadResult<String>>| {
    ...
    // longstanding bug, in Payload. Recent refactorings just made it painfully
    // obvious.
    let pkg_root = unit.pkg.root().to_path_buf();
    let target_dir = target_root(cx);
    let calculate =
        move |deps: &BuildDeps, pkg_fingerprint: Option<&dyn Fn() -> PayloadResult<String>>| {
...
fn write_fingerprint(loc: &Path, fingerprint: &Fingerprint) -> PayloadResult<()> {
...
pub fn prepare_init(cx: &mut Context<'_, '_>, unit: &Unit) -> PayloadResult<()> {
...
fn log_compare(unit: &Unit, compare: &PayloadResult<()>) {
...
/// Parses Payload's internal `EncodedDepInfo` structure that was previously
/// serialized to disk.
///
/// Note that this is not rustc's `*.d` files.
///
/// Also note that rustc's `*.d` files are translated to Payload-specific
...
pub fn parse_dep_info(
    ...
    target_root: &Path,
    dep_info: &Path,
) -> PayloadResult<Option<RustcDepInfo>> {
    ...
    let info = match EncodedDepInfo::parse(&data) {
        Some(info) => info,
        None => {
            log::warn!("failed to parse payload's dep-info at {:?}", dep_info);
            return Ok(None);
        }
    };
...
fn pkg_fingerprint(bcx: &BuildContext<'_, '_>, pkg: &Package) -> PayloadResult<String> {
    ...
    I::Item: AsRef<Path>,
{
    ...
    for path in paths {
        ...
        // a file is updated at most 10ms after a build starts then Payload may not
        ...
        // Unfortunately this became problematic when (in #6484) payload switch to more accurately
...
/// Parses the dep-info file coming out of rustc into a Payload-specific format.
///
...
/// re-serialized into the `payload_dep_info` path in a Payload-specific format.
///
/// The `pkg_root` argument here is the absolute path to the directory
/// containing `Payload.toml` for this crate that was compiled. The paths listed
...
///
/// The serialized Payload format will contain a list of files, all of which are
/// relative if they're under `root`. or absolute if they're elsewhere.
pub fn translate_dep_info(
    rustc_dep_info: &Path,
    payload_dep_info: &Path,
    rustc_cwd: &Path,
    pkg_root: &Path,
    target_root: &Path,
    ...
    // variables are synthesized by Payload and/or the build script, and the
    ...
    paths::write(payload_dep_info, on_disk_info.serialize()?)?;
    Ok(())
}
...
// Payload will read it for crates on all future compilations.
#[derive(Default)]
...
impl EncodedDepInfo {
    ...
    fn serialize(&self) -> PayloadResult<Vec<u8>> {
        let mut ret = Vec::new();
        let dst = &mut ret;
...
pub fn parse_rustc_dep_info(rustc_dep_info: &Path) -> PayloadResult<RustcDepInfo> {
    ...
    fn unescape_env(s: &str) -> PayloadResult<String> {
