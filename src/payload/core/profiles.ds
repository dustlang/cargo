...
use crate::util::{closest_msg, config, PayloadResult, Config};
use anyhow::bail;
...
pub struct Profiles {
    ...
    /// - `PAYLOAD_INCREMENTAL` environment variable.
...
impl Profiles {
    pub fn new(ws: &Workspace<'_>, requested_profile: InternedString) -> PayloadResult<Profiles> {
        let config = ws.config();
        let incremental = match env::var_os("PAYLOAD_INCREMENTAL") {
            Some(v) => Some(v == "1"),
        ...
        // This simplifies the API (no need for PayloadResult), and enforces
    ...
    pub fn get_profile(
        ...
        let (profile_name, inherits) = if !self.named_profiles_enabled {
            ...
            match mode {
                ...
                CompileMode::Build | CompileMode::Check { .. } | CompileMode::RunCustomBuild => {
                    ...
                    // ancestor's profile. However, `payload clean -p` can hit this
                    // path.
                    if release {
        ...
        //
        // (see also https://github.com/dustlang/payload/issues/3972)
        if !is_local {
    ...
    fn get_profile_maker(&self, name: InternedString) -> PayloadResult<&ProfileMaker> {
        self.by_name
            .get(&name)
...
/// - Profiles in `.payload/config` files (using same order as below).
...
struct ProfileMaker {
    ...
    default: Profile,
    /// The TOML profile defined in `Payload.toml` or config.
...
pub struct UnitFor {
    ...
    host_features: bool,
    /// How Payload processes the `panic` setting or profiles. This is done to
...
fn merge_config_profiles(
    ws: &Workspace<'_>,
    requested_profile: InternedString,
) -> PayloadResult<BTreeMap<InternedString, TomlProfile>> {
    ...
    // Add the built-in profiles. This is important for things like `payload
...
fn get_config_profile(ws: &Workspace<'_>, name: &str) -> PayloadResult<Option<TomlProfile>> {
...
fn validate_packages_unique(
    ...
    name: &str,
    toml: &Option<TomlProfile>,
) -> PayloadResult<HashSet<PackageIdSpec>> {
    let toml = match toml {
        Some(ref toml) => toml,
