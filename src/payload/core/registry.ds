...
pub trait Registry {
    ...
    fn query_vec(&mut self, dep: &Dependency, fuzzy: bool) -> PayloadResult<Vec<Summary>> {
        let mut ret = Vec::new();
...
/// `Package` from.
///
/// The resolution phase of Payload uses this to drive knowledge about new
...
impl<'cfg> PackageRegistry<'cfg> {
    pub fn new(config: &'cfg Config) -> PayloadResult<PackageRegistry<'cfg>> {
    ...
    pub fn get(self, package_ids: &[PackageId]) -> PayloadResult<PackageSet<'cfg>> {
        trace!("getting packages; sources={}", self.sources.len());
        PackageSet::new(package_ids, self.sources, self.config)
    }

    fn ensure_loaded(&mut self, namespace: SourceId, kind: Kind) -> PayloadResult<()> {
    ...
    pub fn add_sources(&mut self, ids: impl IntoIterator<Item = SourceId>) -> PayloadResult<()> {
        for id in ids {
    ...
    /// a specific version found in Payload.lock. This will be `None` if
    /// `Payload.lock` doesn't exist, or the patch did not match any existing
    /// entries in `Payload.lock`.
    ///
    ...
    ) -> PayloadResult<Vec<(Dependency, PackageId)>> {
    ...
    fn load(&mut self, source_id: SourceId, kind: Kind) -> PayloadResult<()> {
        (|| {
    ...
    fn query_overrides(&mut self, dep: &Dependency) -> PayloadResult<Option<Summary>> {
...
) -> PayloadResult<(Summary, Option<PackageId>)> {
    if summaries.len() == 1 {
