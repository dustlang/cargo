...
use crate::util::errors::{PayloadResult, PayloadResultExt, HttpNot200};
...
#
# When uploading crates to the registry Payload will automatically
# \"normalize\" Payload.toml files for maximal compatibility
# with all versions of Payload and also rewrite `path` dependencies
# to registry (e.g., crates.io) dependencies
#
# If you believe there's an error in this file please file an
# issue against the dustlang/payload repository. If you're
# editing this file be aware that the upstream Payload.toml
# will likely look very different (and much more reasonable)
";

/// Information about a package that is available somewhere in the file system.
///
/// A package is a `Payload.toml` file plus all the files that are part of it.
//
...
impl Package {
    ...
    pub fn to_registry_toml(&self, ws: &Workspace<'_>) -> PayloadResult<String> {
        let manifest = self
            .manifest()
            .original()
    ...
    /// Returns if package should include `Payload.lock`.
...
pub struct Downloads<'a, 'cfg> {
    ...
    /// Global filesystem lock to ensure only one Payload is downloading at a
    /// time.
...
impl<'cfg> PackageSet<'cfg> {
    pub fn new(
        ...
        // We've enabled the `http2` feature of `curl` in Payload, so treat
    ...
    pub fn enable_download<'a>(&'a self) -> PayloadResult<Downloads<'a, 'cfg>> {
    ...
    pub fn get_one(&self, id: PackageId) -> PayloadResult<&Package> {
    ...
    pub fn get_many(&self, ids: impl IntoIterator<Item = PackageId>) -> PayloadResult<Vec<&Package>> {
    ...
    pub fn download_accessible(
        ...
        // messages for Payload's testsuite. Perhaps there is a better ordering
...
impl<'a, 'cfg> Downloads<'a, 'cfg> {
    ...
    pub fn start(&mut self, id: PackageId) -> PayloadResult<Option<&'a Package>> {
        self.start_inner(id)
            .chain_err(|| format!("failed to download `{}`", id))
    }

    fn start_inner(&mut self, id: PackageId) -> PayloadResult<Option<&'a Package>> {
        ...
        //
        // Currently Payload requests the `http2` feature of the `curl` crate
        // which means it should always be built in. On OSX, however, we ship
        // payload still linked against the system libcurl. Building curl with
        // ALPN support for HTTP/2 requires newer versions of OSX (the
        // SecureTransport API) than we want to ship Payload for. By linking Payload
    ...
    pub fn wait(&mut self) -> PayloadResult<&'a Package> {
        let (dl, data) = loop {
    ...
    fn enqueue(&mut self, dl: Download<'cfg>, handle: Easy) -> PayloadResult<()> {
    ...
    fn wait_for_curl(&mut self) -> PayloadResult<(usize, Result<(), curl::Error>)> {
    ...
    fn tick(&self, why: WhyTick<'_>) -> PayloadResult<()> {
