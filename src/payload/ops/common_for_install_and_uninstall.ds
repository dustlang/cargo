...
/// is both backwards and forwards compatible. Payload keeps both files in sync,
/// updating both v1 and v2 at the same time. Additionally, if it detects
/// changes in v1 that are not in v2 (such as when an older version of Payload
/// is used), it will automatically propagate those changes to v2.
///
/// This maintains a filesystem lock, preventing other instances of Payload from
...
struct CrateListingV2 {
    ...
    /// Forwards compatibility. Unknown keys from future versions of Payload
...
/// then Payload will inform the user that it is "up to date".
///
...
impl InstallTracker {
    ...
    pub fn load(config: &Config, root: &Filesystem) -> PayloadResult<InstallTracker> {
    ...
    pub fn check_upgrade(
        ...
        target: &str,
        _rustc: &str,
    ) -> PayloadResult<(Freshness, BTreeMap<String, Option<PackageId>>)> {
        ...
        if matching_duplicates.len() == duplicates.len() {
            ...
            if source_id.is_path() {
                // `payload install --path ...` is always rebuilt.
                return Ok((Freshness::Dirty, duplicates));
            }
    ...
    pub fn save(&self) -> PayloadResult<()> {
...
impl CrateListingV1 {
    ...
    fn save(&self, lock: &FileLock) -> PayloadResult<()> {
...
impl CrateListingV2 {
    ...
    /// i.e., `payload +new install foo ; payload +old install bar ; payload +new install bar`
...
pub fn resolve_root(flag: Option<&str>, config: &Config) -> PayloadResult<Filesystem> {
    let config_root = config.get_path("install.root")?;
    Ok(flag
        .map(PathBuf::from)
        .or_else(|| env::var_os("PAYLOAD_INSTALL_ROOT").map(PathBuf::from))
...
pub fn path_source(source_id: SourceId, config: &Config) -> PayloadResult<PathSource<'_>> {
    let path = source_id
        .url()
        .to_file_path()
...
where
    T: Source,
    F: FnMut(&mut T) -> PayloadResult<Vec<Package>>,
{
    // This operation may involve updating some sources or making a few queries
    // which may involve frobbing caches, as a result make sure we synchronize
    // with other global Payloads
...
fn one<I, F>(mut i: I, f: F) -> PayloadResult<Option<I::Item>>
where
    I: Iterator,
