...
pub struct WorkspaceResolve<'cfg> {
    ...
    ///
    /// This may be `None` for things like `payload install` and `-Zavoid-dev-deps`.
...
what is locked in the Payload.lock file, run `payload update` to use the new
...
/// (unless it is an ephemeral workspace such as `payload install` or `payload
/// package`).
///
/// This is a simple interface used by commands like `clean`, `fetch`, and
/// `package`, which don't specify any options or features.
pub fn resolve_ws<'a>(ws: &Workspace<'a>) -> PayloadResult<(PackageSet<'a>, Resolve)> {
...
///
/// This function will also write the result of resolution as a new lock file
/// (unless `Workspace::require_optional_deps` is false, such as `payload
/// install` or `-Z avoid-dev-deps`), or it is an ephemeral workspace (`payload
/// install` or `payload package`).
///
...
) -> PayloadResult<WorkspaceResolve<'cfg>> {
    let mut registry = PackageRegistry::new(ws.config())?;
...
pub fn resolve_with_previous<'cfg>(
    ...
    // We only want one Payload at a time resolving a crate graph since this can
...
pub fn add_overrides<'a>(
    ...
    let paths = paths.val.iter().map(|(s, def)| {
        ...
        // key was located, so we want to pop off the `.payload/config` component
        // to get the directory containing the `.payload` folder.
...
fn register_previous_locks(
    ...
    let path_pkg = |id: SourceId| {
        ...
        if let Ok(path) = id.url().to_file_path() {
            if let Ok(pkg) = ws.load(&path.join("Payload.toml")) {
                return Some(pkg);
            }
        }
        None
    ...
    // line". That is, if we run `payload update -p foo` then `keep(bar)` will return
    ...
    //
    // Let's say we then run `payload update -p serde`. This may *also* want to
    ...
    // graph can come from two locations, the `payload update` command or
    ...
    for node in resolve.iter().filter(keep) {
        ...
        // directive. Nowadays in Payload, however, these two directives are
