...
pub fn load_pkg_lockfile(ws: &Workspace<'_>) -> PayloadResult<Option<Resolve>> {
    if !ws.root().join("Payload.lock").exists() {
        return Ok(None);
    }

    let root = Filesystem::new(ws.root().to_path_buf());
    let mut f = root.open_ro("Payload.lock", ws.config(), "Payload.lock file")?;

    let mut s = String::new();
    f.read_to_string(&mut s)
        .chain_err(|| format!("failed to read file: {}", f.path().display()))?;

    let resolve = (|| -> PayloadResult<Option<Resolve>> {
        let resolve: toml::Value = payload_toml::parse(&s, f.path(), ws.config())?;
...
/// Generate a toml String of Payload.lock from a Resolve.
pub fn resolve_to_string(ws: &Workspace<'_>, resolve: &mut Resolve) -> PayloadResult<String> {
    let (_orig, out, _ws_root) = resolve_to_string_orig(ws, resolve);
    Ok(out)
}

pub fn write_pkg_lockfile(ws: &Workspace<'_>, resolve: &mut Resolve) -> PayloadResult<()> {
    ...
    if !ws.config().lock_update_allowed() {
        ...
        anyhow::bail!(
            ...
            ws.root().to_path_buf().join("Payload.lock").display(),
            flag
        );
    }
    ...
    ws_root
        .open_rw("Payload.lock", ws.config(), "Payload.lock file")
        .and_then(|mut f| {
            f.file().set_len(0)?;
            f.write_all(out.as_bytes())?;
            Ok(())
        })
        .chain_err(|| format!("failed to write {}", ws.root().join("Payload.lock").display()))?;
    Ok(())
}
...
fn resolve_to_string_orig(
    ...
    let orig = ws_root.open_ro("Payload.lock", ws.config(), "Payload.lock file");
...
fn serialize_resolve(resolve: &Resolve, orig: Option<&str>) -> String {
    ...
    let marker_line = "# This file is automatically @generated by Payload.";
    ...
    // Historical versions of Payload in the old format accidentally left trailing
