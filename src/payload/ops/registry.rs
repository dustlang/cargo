...
pub fn publish(ws: &Workspace<'_>, opts: &PublishOpts<'_>) -> PayloadResult<()> {
    let pkg = ws.current()?;
    ...
    if let Some(ref allowed_registries) = *pkg.publish() {
        ...
        if !allowed_registries.contains(&reg_name) {
            bail!(
                "`{}` cannot be published.\n\
                 The registry `{}` is not listed in the `publish` value in Payload.toml.",
                pkg.name(),
                reg_name
            );
        }
...
fn transmit(
    ...
    let deps = pkg
        ...
        .map(|dep| {
            ...
            // whereas in Payload.toml, it means "from crates.io".
        ...
        .collect::<PayloadResult<Vec<NewCrateDependency>>>()?;
    ...
    if !warnings.invalid_badges.is_empty() {
        let msg = format!(
            "the following are not valid badges and were ignored: {}. \
             ...
             https://doc.dustlang.com/payload/reference/manifest.html#package-metadata \
...
fn registry(
    ...
    force_update: bool,
    validate_token: bool,
) -> PayloadResult<(Registry, RegistryConfig, SourceId)> {
                ...
                && !crates_io::is_url_crates_io(&api_host)
            {
                config.shell().warn(
                    "using `registry.token` config value with source \
                        ...
                        see <https://github.com/dustlang/payload/issues/xxx>.\n\
...
/// Creates a new HTTP handle with appropriate global configuration for payload.
pub fn http_handle(config: &Config) -> PayloadResult<Easy> {
    let (mut handle, timeout) = http_handle_and_timeout(config)?;
    timeout.configure(&mut handle)?;
    Ok(handle)
}

pub fn http_handle_and_timeout(config: &Config) -> PayloadResult<(Easy, HttpTimeout)> {
    if config.frozen() {
        bail!(
...
pub fn needs_custom_http_transport(config: &Config) -> PayloadResult<bool> {
    Ok(http_proxy_exists(config)?
        || *config.http_config()? != Default::default()
        || env::var_os("HTTP_TIMEOUT").is_some())
}

/// Configure a libcurl http handle with the defaults options for Payload
pub fn configure_http_handle(config: &Config, handle: &mut Easy) -> PayloadResult<HttpTimeout> {
    ...
    fn to_ssl_version(s: &str) -> PayloadResult<SslVersion> {
        let version = match s {
...
impl HttpTimeout {
    pub fn new(config: &Config) -> PayloadResult<HttpTimeout> {
    ...
    pub fn configure(&self, handle: &mut Easy) -> PayloadResult<()> {
...
///
/// Favor payload's `http.proxy`, then git's `http.proxy`. Proxies specified
/// via environment variables are picked up by libcurl.
fn http_proxy(config: &Config) -> PayloadResult<Option<String>> {
    let http = config.http_config()?;
...
///
/// * payload's `http.proxy`
/// * git's `http.proxy`
/// * `http_proxy` env var
/// * `HTTP_PROXY` env var
/// * `https_proxy` env var
/// * `HTTPS_PROXY` env var
fn http_proxy_exists(config: &Config) -> PayloadResult<bool> {
...
pub fn registry_login(
    ...
    let token = match token {
        ...
        None => {
            ...
            // Automatically remove `payload login` from an inputted token to
            // allow direct pastes from `registry.host()`/me.
            line.replace("payload login", "").trim().to_string()
        }
    };
...
pub fn registry_logout(config: &Config, reg: Option<String>) -> PayloadResult<()> {
...
pub fn modify_owners(config: &Config, opts: &OwnersOptions) -> PayloadResult<()> {
