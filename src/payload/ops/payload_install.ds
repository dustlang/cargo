...
fn install_one(
        ...
        if name == "." {
            bail!(
                "To install the binaries for the package in current working \
                 directory use `payload install --path .`. \
    ...
    let pkg = {
        ...
        if source_id.is_git() {
            ...
            if !src.path().is_dir() {
                bail!(
                    "`{}` is not a directory. \
                 --path must point to a directory containing a Payload.toml file.",
                    src.path().display()
                )
            }
            if !src.path().join("Payload.toml").exists() {
                if from_cwd {
                    bail!(
                    ...
                    bail!(
                        "`{}` does not contain a Payload.toml file. \
                     --path must point to a directory containing a Payload.toml file.",
                        src.path().display()
                    )
                }
    ...
    // If we're installing in --locked mode and there's no `Payload.lock` published
    // ie. the bin was published before https://github.com/dustlang/payload/pull/7026
    if config.locked() && !ws.root().join("Payload.lock").exists() {
        config.shell().warn(format!(
            "no Payload.lock file published in {}",
            pkg.to_string()
        ))?;
    }
    ...
    if !source_id.is_path() {
        let target_dir = if let Some(dir) = config.target_dir()? {
            dir
        } else if let Ok(td) = TempFileBuilder::new().prefix("payload-install").tempdir() {
    ...
    if from_cwd {
        if pkg.manifest().edition() == Edition::Edition2015 {
            config.shell().warn(
                "Using `payload install` to install the binaries for the \
    ...
    // For bare `payload install` (no `--bin` or `--example`), check if there is
    ...
    if !opts.filter.is_specific() && !pkg.targets().iter().any(|t| t.is_bin()) {
        bail!(
            "there is nothing to install in `{}`, because it has no binaries\n\
             `payload install` is only for installing programs, and can't be used with libraries.\n\
             To use a library crate, add it as a dependency in a Payload project instead.",
            pkg
        );
    }

    // Helper for --no-track flag to make sure it doesn't overwrite anything.
    let no_track_duplicates = || -> PayloadResult<BTreeMap<String, Option<PackageId>>> {
    ...
    let replace_result = {
        let mut try_install = || -> PayloadResult<()> {
...
fn make_ws_rustc_target<'cfg>(
    ...
    source_id: &SourceId,
    pkg: Package,
) -> PayloadResult<(Workspace<'cfg>, Rustc, String)> {
...
/// values.
fn parse_semver_flag(v: &str) -> PayloadResult<VersionReq> {
        ...
        match v.parse::<VersionReq>() {
            ...
            Err(_) => bail!(
                "the `--vers` provided, `{}`, is \
                     ...
                     https://doc.dustlang.com/payload/reference/specifying-dependencies.html \
...
fn check_yanked_install(ws: &Workspace<'_>) -> PayloadResult<()> {
    if ws.ignore_lock() || !ws.root().join("Payload.lock").exists() {
        return Ok(());
    }
        ...
        if let Some(source) = sources.get_mut(pkg_id.source_id()) {
            if source.is_yanked(pkg_id)? {
                ws.config().shell().warn(format!(
                    "package `{}` in Payload.lock is yanked in registry `{}`, \
...
pub fn install_list(dst: Option<&str>, config: &Config) -> PayloadResult<()> {
