...
pub fn read_package(
    ...
    source_id: SourceId,
    config: &Config,
) -> PayloadResult<(Package, Vec<PathBuf>)> {
    trace!(
...
fn walk(path: &Path, callback: &mut dyn FnMut(&Path) -> PayloadResult<bool>) -> PayloadResult<()> {
    if !callback(path)? {
...
fn has_manifest(path: &Path) -> bool {
    find_project_manifest_exact(path, "Payload.toml").is_ok()
}
...
fn read_nested_packages(
    ...
    let manifest_path = find_project_manifest_exact(path, "Payload.toml")?;
    ...
    if !source_id.is_registry() {
        for p in nested.iter() {
            ...
            //
            // See https://github.com/dustlang/payload/issues/6822.
