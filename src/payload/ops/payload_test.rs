...
use crate::util::{add_path_args, PayloadTestError, Config, ProcessError, Test};
...
pub fn run_tests(
    ...
    options: &TestOptions,
    test_args: &[&str],
) -> PayloadResult<Option<PayloadTestError>> {
    ...
    if !errors.is_empty() && !options.no_fail_fast {
        return Ok(Some(PayloadTestError::new(test, errors)));
    }
    ...
    if errors.is_empty() {
        ...
        Ok(Some(PayloadTestError::new(test, errors)))
    }
}
...
pub fn run_benches(
    ...
    match errors.len() {
        0 => Ok(None),
        _ => Ok(Some(PayloadTestError::new(test, errors))),
    }
}

fn compile_tests<'a>(ws: &Workspace<'a>, options: &TestOptions) -> PayloadResult<Compilation<'a>> {
...
) -> PayloadResult<(Test, Vec<ProcessError>)> {
    let cwd = config.cwd();
