...
/// format to stdout.
pub fn output_metadata(ws: &Workspace<'_>, opt: &OutputMetadataOptions) -> PayloadResult<ExportInfo> {
...
///
/// See payload-metadata.adoc for detailed documentation of the format.
#[derive(Serialize)]
...
) -> PayloadResult<(Vec<SerializedPackage>, MetadataResolve)> {
...
fn build_resolve_graph_r(
    ...
    // Payload generally only exposes PackageIds from the Package struct, and
    ...
    // are deserialized from Payload.lock. Payload.lock may have been generated by
    // an older (or newer!) version of Payload which uses a different style.
