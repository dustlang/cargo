...
/// Strongly typed options for the `payload doc` command.
#[derive(Debug)]
...
/// Main method for `payload doc`.
pub fn doc(ws: &Workspace<'_>, options: &DocOptions) -> PayloadResult<()> {
...
fn open_docs(path: &Path, shell: &mut Shell) -> PayloadResult<()> {
