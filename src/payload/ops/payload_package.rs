...
const VCS_INFO_FILE: &str = ".payload_vcs_info.json";
...
enum GeneratedFile {
    /// Generates `Payload.toml` by rewriting the original.
    Manifest,
    /// Generates `Payload.lock` in some cases (like if there is a binary).
    Lockfile,
    /// Adds a `.payload-vcs_info.json` file if in a (clean) git repo.
    VcsInfo(String),
}

pub fn package(ws: &Workspace<'_>, opts: &PackageOpts<'_>) -> PayloadResult<Option<FileLock>> {
    if ws.root().join("Payload.lock").exists() {
        // Make sure the Payload.lock is up-to-date and valid.
        let _ = ops::resolve_ws(ws)?;
        // If Payload.lock does not exist, it will be generated by `build_lock`
...
fn build_ar_list(
        ...
        match rel_str.as_ref() {
            "Payload.toml" => {
                result.push(ArchiveFile {
                    rel_path: PathBuf::from("Payload.toml.orig"),
                    rel_str: "Payload.toml.orig".to_string(),
    ...
    if pkg.include_lockfile() {
        result.push(ArchiveFile {
            rel_path: PathBuf::from("Payload.lock"),
            rel_str: "Payload.lock".to_string(),
...
/// Construct `Payload.lock` for the package to be published.
fn build_lock(ws: &Workspace<'_>) -> PayloadResult<String> {
    let config = ws.config();
    ...
    // Regenerate Payload.lock using the old one as a guide.
...
fn check_metadata(pkg: &Package, config: &Config) -> PayloadResult<()> {
...
fn verify_dependencies(pkg: &Package) -> PayloadResult<()> {
...
fn check_repo_state(
        ...
        if let Some(workdir) = repo.workdir() {
            ...
            if let Ok(status) = repo.status_file(path) {
                if (status & git2::Status::IGNORED).is_empty() {
                    debug!(
                        "found (git) Payload.toml at {:?} in workdir {:?}",
                        path, workdir
                    );
            ...
            config.shell().verbose(|shell| {
                shell.warn(format!(
                    "No (git) Payload.toml found at `{}` in workdir `{}`",
                    path.display(),
                    workdir.display()
                ))
            })?;
    ...
    // No VCS with a checked in `Payload.toml` found, so we don't know if the
...
/// Generate warnings when packaging Payload.lock, and the resolve have changed.
fn compare_resolve(
    config: &Config,
    current_pkg: &Package,
    ...
    for pkg_id in added {
        ...
        let extra = match removed_candidates.len() {
            ...
            1 => {
                ...
                // `[replace]` is not captured in Payload.lock.
                format!(
        ...
        let msg = format!(
            "package `{}` added to the packaged Payload.lock file{}",
            pkg_id, extra
        );
...
fn check_yanked(config: &Config, pkg_set: &PackageSet<'_>, resolve: &Resolve) -> PayloadResult<()> {
...
fn run_verify(ws: &Workspace<'_>, tar: &FileLock, opts: &PackageOpts<'_>) -> PayloadResult<()> {
    let config = ws.config();
    let pkg = ws.current()?;
...
fn hash_all(path: &Path) -> PayloadResult<HashMap<PathBuf, u64>> {
    fn wrap(path: &Path) -> PayloadResult<HashMap<PathBuf, u64>> {
...
fn check_filename(file: &Path, shell: &mut Shell) -> PayloadResult<()> {
