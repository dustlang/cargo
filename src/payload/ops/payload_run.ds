...
pub fn run(
    ...
    if options.filter.contains_glob_patterns() {
        anyhow::bail!("`payload run` does not support glob patterns on target selection")
    }
    ...
    if bins.is_empty() {
        if !options.filter.is_specific() {
            anyhow::bail!("a bin target must be available for `payload run`")
        } else {
            // This will be verified in `payload_compile`.
        }
    }
    ...
    if bins.len() > 1 {
        if !options.filter.is_specific() {
            ...
            names.sort();
            anyhow::bail!(
                "`payload run` could not determine which binary to run. \
            ...
            anyhow::bail!(
                "`payload run` can run at most one executable, but \
                 multiple were specified"
            )
        }
    }

    // `payload run` is only compatible with one `--target` flag at most
