...
use crate::util::{config, process_error, PayloadResult, PayloadResultExt, Config};
use anyhow::bail;
...
pub(super) fn auth_token(
    ...
    let token = match (cli_token, config_token, credential_process) {
        (None, None, None) => {
            bail!("no upload token found, please run `payload login` or pass `--token`");
        }
...
fn run_command(
    ...
    api_url: &str,
    action: Action,
) -> PayloadResult<Option<String>> {
    let cred_proc;
    let (exe, args) = if process.0.to_str().unwrap_or("").starts_with("payload:") {
    ...
    cmd.args(args)
        .env("PAYLOAD", config.payload_exe()?)
        .env("PAYLOAD_REGISTRY_NAME", name)
        .env("PAYLOAD_REGISTRY_API_URL", api_url);
    match action {
        Action::Get => {
...
fn sysroot_credential(
    config: &Config,
    process: &(PathBuf, Vec<String>),
) -> PayloadResult<(PathBuf, Vec<String>)> {
    let cred_name = process.0.to_str().unwrap().strip_prefix("payload:").unwrap();
    let payload = config.payload_exe()?;
    let root = payload
        .parent()
        .and_then(|p| p.parent())
        .ok_or_else(|| format_err!("expected payload path {}", payload.display()))?;
