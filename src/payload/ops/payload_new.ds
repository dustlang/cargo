...
fn get_name<'a>(path: &'a Path, opts: &'a NewOptions) -> PayloadResult<&'a str> {
...
pub fn new(opts: &NewOptions, config: &Config) -> PayloadResult<()> {
    let path = &opts.path;
    if path.exists() {
        anyhow::bail!(
...
pub fn init(opts: &NewOptions, config: &Config) -> PayloadResult<()> {
    // This is here just as a random location to exercise the internal error handling.
    if std::env::var_os("__PAYLOAD_TEST_INTERNAL_ERROR").is_some() {
        return Err(crate::util::internal("internal error test"));
    }

    let path = &opts.path;

    if path.join("Payload.toml").exists() {
        anyhow::bail!("`payload init` cannot be run on existing Payload packages")
    }
    ...
    if src_paths_types.is_empty() {
        ...
        // Maybe when doing `payload init --bin` inside a library package stub,
    ...
    if version_control == None {
        ...
        // if none exists, maybe create git, like in `payload new`
...
fn init_vcs(path: &Path, vcs: VersionControl, config: &Config) -> PayloadResult<()> {
    match vcs {
        VersionControl::Git => {
            if !path.join(".git").exists() {
...
fn mk(config: &Config, opts: &MkOptions<'_>) -> PayloadResult<()> {
    let path = opts.path;
    let name = opts.name;
    let cfg = config.get::<PayloadNewConfig>("payload-new")?;
    ...
    if !opts.bin {
        ignore.push("Payload.lock", "glob:Payload.lock");
    }
    ...
    let mut payloadtoml_path_specifier = String::new();

    // Calculate what `[lib]` and `[[bin]]`s we need to append to `Payload.toml`.

    for i in &opts.source_files {
        if i.bin {
            if i.relative_path != "src/main.rs" {
                payloadtoml_path_specifier.push_str(&format!(
                    r#"
[[bin]]
name = "{}"
path = {}
...
"#,
    ...
    // Create `Payload.toml` file with necessary `[lib]` and `[[bin]]` sections, if needed.

    paths::write(
        &path.join("Payload.toml"),
        format!(
            r#"[package]
name = "{}"
version = "0.1.0"
...
edition = {}
{}
# See more keys and their definitions at https://doc.dustlang.com/payload/reference/manifest.html
...
"
    ...
    if let Err(e) = Workspace::new(&path.join("Payload.toml"), config) {
...
fn find_git_config(path: &Path) -> Option<GitConfig> {
    match env::var("__PAYLOAD_TEST_ROOT") {
