...
use crate::util::{closest_msg, profile, PayloadResult, StableHasher};
...
pub struct CompileOptions {
    ...
    /// defined in the payload metadata for a crate.
    pub honor_rust_version: bool,
}

impl<'a> CompileOptions {
    pub fn new(config: &Config, mode: CompileMode) -> PayloadResult<CompileOptions> {
        Ok(CompileOptions {
...
impl Packages {
    pub fn from_flags(all: bool, exclude: Vec<String>, package: Vec<String>) -> PayloadResult<Self> {
    ...
    pub fn to_package_id_specs(&self, ws: &Workspace<'_>) -> PayloadResult<Vec<PackageIdSpec>> {
    ...
    pub fn get_packages<'ws>(&self, ws: &'ws Workspace<'_>) -> PayloadResult<Vec<&'ws Package>> {
...
pub fn compile<'a>(ws: &Workspace<'a>, options: &CompileOptions) -> PayloadResult<Compilation<'a>> {
...
pub fn create_bcx<'a, 'cfg>(
...
) -> PayloadResult<BuildContext<'a, 'cfg>> {
    let CompileOptions {
        ref build_config,
        ref spec,
        ref features,
    ...
    match build_config.mode {
        ...
        | CompileMode::RunCustomBuild => {
            if std::env::var("RUST_FLAGS").is_ok() {
                config.shell().warn(
                    "Payload does not read `RUST_FLAGS` environment variable. Did you mean `RUSTFLAGS`?",
                )?;
            }
        }
        CompileMode::Doc { .. } | CompileMode::Doctest => {
            if std::env::var("RUSTDOC_FLAGS").is_ok() {
                config.shell().warn(
                    "Payload does not read `RUSTDOC_FLAGS` environment variable. Did you mean `RUSTDOCFLAGS`?"
                )?;
            }
        }
    }
    ...
    // The ordering here affects some error messages coming out of payload, so
    ...
    // TODO: In theory, Payload should also dedupe the roots, but I'm uncertain
...
impl CompileFilter {
    ...
    // this selects targets for "payload run". for logic to select targets for
    ...
    let new_unit =
        |units: &mut HashSet<Unit>, pkg: &Package, target: &Target, target_mode: CompileMode| {
            let unit_for = if target_mode.is_any_test() {
                ...
                //
                // Forcing the lib to be compiled three times during `payload
...
/// collision in the `UnitGraph`, and Payload would end up randomly choosing one
/// value or the other.
///
...
fn build_glob(pat: &str) -> PayloadResult<glob::Pattern> {
...
fn opt_patterns_and_names(
    opt: &[String],
) -> PayloadResult<(Vec<(glob::Pattern, bool)>, BTreeSet<&str>)> {
