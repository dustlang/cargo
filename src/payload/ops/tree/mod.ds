//! Implementation of `payload tree`.
...
/// Entry point for the `payload tree` command.
pub fn build_and_print(ws: &Workspace<'_>, opts: &TreeOptions) -> PayloadResult<()> {
    ...
    let root_specs = if opts.invert.is_empty() {
        ...
        opts.invert
            ...
            .collect::<PayloadResult<Vec<PackageIdSpec>>>()?
    };
