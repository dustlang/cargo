...
//!
//! The main goal is to run `payload check` to get rustc to emit JSON
...
//!
//! Payload begins by launching a `LockServer` thread in the background to
...
//!
//! Payload begins a normal `payload check` operation with itself set as a proxy
//! for rustc by setting `primary_unit_rustc` in the build config. When
//! payload launches rustc to check a crate, it is actually launching itself.
//! The `FIX_ENV` environment variable is set so that payload knows it is in
//! fix-proxy-mode.
//!
//! Each proxied payload-as-rustc detects it is in fix-proxy-mode (via `FIX_ENV`
//! environment variable in `main`) and does the following:
//!
//! - Acquire a lock from the `LockServer` from the master payload process.
...
const FIX_ENV: &str = "__PAYLOAD_FIX_PLZ";
const BROKEN_CODE_ENV: &str = "__PAYLOAD_FIX_BROKEN_CODE";
const EDITION_ENV: &str = "__PAYLOAD_FIX_EDITION";
const IDIOMS_ENV: &str = "__PAYLOAD_FIX_IDIOMS";
...
pub fn fix(ws: &Workspace<'_>, opts: &mut FixOptions) -> PayloadResult<()> {
    ...
    *opts
    ...
    // primary crates are compiled using a payload subprocess to do extra work of applying fixes and
...
fn check_version_control(config: &Config, opts: &FixOptions) -> PayloadResult<()> {
    if opts.allow_no_vcs {
        return Ok(());
    }
    if !existing_vcs_repo(config.cwd(), config.cwd()) {
        bail!(
            "no VCS found for this package and `payload fix` can potentially \
    ...
    bail!(
        "the working directory of this package has uncommitted changes, and \
         `payload fix` can potentially perform destructive changes; if you'd \
...
fn check_resolver_change(ws: &Workspace<'_>, opts: &FixOptions) -> PayloadResult<()> {
    ...
    // This is not the same as what `payload fix` is doing, since it is doing
    ...
    config.shell().note(
        "Switching to Edition 2021 will enable the use of the version 2 feature resolver in Payload.",
    )?;
    drop_eprintln!(
        config,
    ...
    drop_eprintln!(
        ...
        "More information about the resolver changes may be found \
         at https://doc.dustlang.com/payload/reference/features.html#feature-resolver-version-2"
    );
    drop_eprintln!(
        config,
...
/// Entry point for `payload` running as a proxy for `rustc`.
///
/// This is called every time `payload` is run to check if it is in proxy mode.
///
...
pub fn fix_maybe_exec_rustc(config: &Config) -> PayloadResult<bool> {
    ...
    trace!("payload-fix as rustc got file {:?}", args.file);
    ...
    for arg in args.format_args {
        // Add any json/error format arguments that Payload wants. This allows
...
fn rustfix_crate(
    ...
    // directory.
    let dir = env::var("PAYLOAD_MANIFEST_DIR").expect("PAYLOAD_MANIFEST_DIR is missing?");
    ...
    let iterations = env::var("PAYLOAD_FIX_MAX_RETRIES")
        .ok()
    ...
    // bugs in Payload and/or rustfix.
    for (path, file) in fixes.files.iter_mut() {
...
fn rustfix_and_fix(
    ...
    let fix_mode = env::var_os("__PAYLOAD_FIX_YOLO")
...
/// Various command-line options and settings used when `payload` is running as
...
struct FixArgs {
    ...
    /// Payload normally passes when applying fixes. When displaying warnings or
...
impl FixArgs {
        ...
        for arg in env::args_os().skip(2) {
            ...
            if let Some(s) = path.to_str() {
                ...
                if s.starts_with("--error-format=") || s.starts_with("--json=") {
                    // Payload may add error-format in some cases, but `payload
    ...
    /// done.
    fn check_edition_and_send_status(&self, config: &Config) -> PayloadResult<()> {
        ...
        // Unfortunately determining which payload targets are being built
        // isn't easy, and each target can be a different edition. The
        // payload-as-rustc fix wrapper doesn't know anything about the
        // workspace, so it can't check for the `payload-features` unstable
