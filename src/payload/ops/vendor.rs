...
use crate::util::{paths, PayloadResult, PayloadResultExt, Config};
use anyhow::bail;
use serde::Serialize;
...
pub fn vendor(ws: &Workspace<'_>, opts: &VendorOptions<'_>) -> PayloadResult<()> {
    ...
    if config.shell().verbosity() != Verbosity::Quiet {
        crate::drop_eprint!(
            config,
            "To use vendored sources, add this to your .payload/config.toml for this project:\n\n"
        );
...
fn sync(
    ...
    // First up attempt to work around dustlang/payload#5956. Apparently build
    // artifacts sprout up in Payload's global cache for whatever reason, although
    // it's unsure what tool is causing these issues at this time. For now we
    // apply a heavy-hammer approach which is to delete Payload's unpacked version
    // of each crate to start off with. After we do this we'll re-resolve and
    // redownload again, which should trigger Payload to re-extract all the
    // crates.
    //
    ...
    for (id, pkg) in ids.iter() {
        ...
        let cksum = dst.join(".payload-checksum.json");
...
fn cp_sources(
    ...
    for p in paths {
        ...
        match relative.to_str() {
            ...
            Some(".payload-ok") => continue,

            // Skip patch-style orig/rej files. Published crates on crates.io
            // have `Payload.toml.orig` which we don't want to use here and
...
fn copy_and_checksum(src_path: &Path, dst_path: &Path, buf: &mut [u8]) -> PayloadResult<String> {
