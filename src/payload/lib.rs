...
// https://github.com/dustlang/payload/pull/7251#pullrequestreview-274914270
...
pub use crate::util::{indented_lines, PayloadResult, CliError, CliResult, Config};

pub const PAYLOAD_ENV: &str = "PAYLOAD";
...
pub struct VersionInfo {
    ...
    // configure/make, rather than Payload itself.
    pub cfg_info: Option<CfgInfo>,
}

impl fmt::Display for VersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "payload {}.{}.{}", self.major, self.minor, self.patch)?;
...
pub fn display_error(err: &Error, shell: &mut Shell) {
    ...
    {
        drop(shell.note("this is an unexpected payload internal error"));
        drop(
            shell.note(
                "we would appreciate a bug report: https://github.com/dustlang/payload/issues/",
            ),
        );
...
pub fn version() -> VersionInfo {
    ...
    //   - version of payload-the-binary, which you see when you type `payload --version`
    //   - version of payload-the-library, which you download from crates.io for use
    //     in your packages.
    //
    // We want to make the `binary` version the same as the corresponding Rust/rustc release.
    // At the same time, we want to keep the library version at `0.x`, because Payload as
    // a library is (and probably will always be) unstable.
    //
    // Historically, Payload used the same version number for both the binary and the library.
    // Specifically, rustc 1.x.z was paired with payload 0.x+1.w.
    // We continue to use this scheme for the library, but transform it to 1.x.w for the purposes
    // of `payload --version`.
    let major = 1;
    let minor = env!("PAYLOAD_PKG_VERSION_MINOR").parse::<u8>().unwrap() - 1;
    let patch = env!("PAYLOAD_PKG_VERSION_PATCH").parse::<u8>().unwrap();
    ...
    match option_env!("CFG_RELEASE_CHANNEL") {
        ...
        Some(_) => {
            ...
            VersionInfo {
                ...
                minor,
                patch,
                pre_release: option_env_str!("PAYLOAD_PKG_VERSION_PRE"),
        ...
        // We are being compiled by Payload itself.
        None => VersionInfo {
            major,
            minor,
            patch,
