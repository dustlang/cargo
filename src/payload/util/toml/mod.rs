...
/// Loads a `Payload.toml` from a file on disk.
///
...
fn do_read_manifest(
    ...
    source_id: SourceId,
    config: &Config,
) -> PayloadResult<(EitherManifest, Vec<PathBuf>)> {
    ...
    if let Some(package) = toml.get("package").or_else(|| toml.get("project")) {
        if let Some(feats) = package.get("payload-features") {
            bail!(
                "payload-features = {} was found in the wrong location, it \
                 should be set at the top of Payload.toml before any tables",
...
pub fn parse(toml: &str, file: &Path, config: &Config) -> PayloadResult<toml::Value> {
...
historical versions of Payload have erroneously accepted this file. The table
...
pub struct DetailedTomlDependency<P = String> {
    ...
    /// This is an internal implementation detail. When Payload creates a
    ...
    /// registry names configured, so Payload can't rely on just the name for
    /// crates published by other users.
    registry_index: Option<String>,
    // `path` is relative to the file it appears in. If that's a `Payload.toml`, it'll be relative to
    // that TOML file, and if it's a `.payload/config` file, it'll be relative to that file.
    path: Option<P>,
    git: Option<String>,
...
/// This type is used to deserialize `Payload.toml` files.
#[derive(Debug, Deserialize, Serialize)]
#[serde(rename_all = "kebab-case")]
pub struct TomlManifest {
    payload_features: Option<Vec<String>>,
...
impl TomlProfiles {
    ...
    pub fn validate(&self, features: &Features, warnings: &mut Vec<String>) -> PayloadResult<()> {
...
impl TomlProfile {
    ...
    pub fn validate_name(name: &str, what: &str) -> PayloadResult<()> {
        if let Some(ch) = name
            .chars()
    ...
    fn validate_override(&self, which: &str) -> PayloadResult<()> {
...
/// Represents the `package`/`project` sections of a `Payload.toml`.
///
...
impl TomlProject {
    pub fn to_package_id(&self, source_id: SourceId) -> PayloadResult<PackageId> {
...
impl TomlManifest {
    ...
    pub fn prepare_for_publish(
        ...
        if let Some(license_file) = &package.license_file {
            ...
            if abs_license_path.strip_prefix(package_root).is_err() {
                // This path points outside of the package root. `payload package`
        ...
        return Ok(TomlManifest {
            ...
            payload_features: self.payload_features.clone(),
        });
        ...
        fn map_deps(
        ...
        ) -> PayloadResult<Option<BTreeMap<String, TomlDependency>>> {
            let deps = match deps {
                Some(deps) => deps,
                None => return Ok(None),
            };
            let deps = deps
                ...
                .collect::<PayloadResult<BTreeMap<_, _>>>()?;
            Ok(Some(deps))
        }

        fn map_dependency(config: &Config, dep: &TomlDependency) -> PayloadResult<TomlDependency> {
            match dep {
    ...
    pub fn to_real_manifest(
        ...
        package_root: &Path,
        config: &Config,
    ) -> PayloadResult<(Manifest, Vec<PathBuf>)> {
        ...
        let empty = Vec::new();
        let payload_features = me.payload_features.as_ref().unwrap_or(&empty);
        let features = Features::new(payload_features, config, &mut warnings)?;
        ...
        let rust_version = if let Some(rust_version) = &project.rust_version {
            if features.require(Feature::rust_version()).is_err() {
                let mut msg =
                    "`rust-version` is not supported on this version of Payload and will be ignored"
                        .to_string();
    ...
    fn to_virtual_manifest(
        ...
        root: &Path,
        config: &Config,
    ) -> PayloadResult<(VirtualManifest, Vec<PathBuf>)> {
    ...
    fn replace(&self, cx: &mut Context<'_, '_>) -> PayloadResult<Vec<(PackageIdSpec, Dependency)>> {
    ...
    fn patch(&self, cx: &mut Context<'_, '_>) -> PayloadResult<HashMap<Url, Vec<Dependency>>> {
    ...
    fn maybe_custom_build(
        ...
        match *build {
            ...
            None => {
                // If there is a `build.rs` file next to the `Payload.toml`, assume it is
                // a build script.
                if build_rs.is_file() {
                    Some(build_rs)
                } else {
...
struct TomlTarget {
    ...
    // versions of Payload also accepted `crate_type`, so look for both.
