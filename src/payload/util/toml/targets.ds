//! This module implements Payload conventions for directory layout:
//!
...
//!
//! It is a bit tricky because we need match explicit information from `Payload.toml`
        ...
        if restricted_names::is_conflicting_artifact_name(&name) {
            anyhow::bail!(
                "the binary target name `{}` is forbidden, \
                 it conflicts with with payload's build directory names",
                name
            )
        }
    }
...
) -> PayloadResult<Vec<(PathBuf, TomlTarget)>> {
    clean_targets_with_legacy_path(
        target_kind_human,
        target_kind,
        toml_targets,
                ...
                None => {
                    if edition == Edition::Edition2015 {
                        if !rem_targets.is_empty() {
                            ...
                            warnings.push(format!(
                                "\
An explicit [[{section}]] section is specified in Payload.toml which currently
disables Payload from automatically inferring other {target_kind_human} targets.
...
fn validate_unique_names(targets: &[TomlTarget], target_kind: &str) -> PayloadResult<()> {
...
fn configure(features: &Features, toml: &TomlTarget, target: &mut Target) -> PayloadResult<()> {
    let t2 = target.clone();
    target
