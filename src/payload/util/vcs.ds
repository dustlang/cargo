...
use crate::util::{process, PayloadResult};
use std::path::Path;
...
impl GitRepo {
    pub fn init(path: &Path, _: &Path) -> PayloadResult<GitRepo> {
...
impl HgRepo {
    pub fn init(path: &Path, cwd: &Path) -> PayloadResult<HgRepo> {
        process("hg").cwd(cwd).arg("init").arg(path).exec()?;
        Ok(HgRepo)
    }
    pub fn discover(path: &Path, cwd: &Path) -> PayloadResult<HgRepo> {
        process("hg")
            .cwd(cwd)
            .arg("--cwd")
            .arg(path)
...
impl PijulRepo {
    pub fn init(path: &Path, cwd: &Path) -> PayloadResult<PijulRepo> {
...
impl FossilRepo {
    pub fn init(path: &Path, cwd: &Path) -> PayloadResult<FossilRepo> {
