...
pub fn join_paths<T: AsRef<OsStr>>(paths: &[T], env: &str) -> PayloadResult<OsString> {
...
pub fn dylib_path_envvar() -> &'static str {
    if cfg!(windows) {
        ...
        // penalty starting in 10.13. Payload's testsuite ran more than twice as
        // slow with it on CI.
        "DYLD_FALLBACK_LIBRARY_PATH"
    } else {
        "LD_LIBRARY_PATH"
...
pub fn resolve_executable(exec: &Path) -> PayloadResult<PathBuf> {
...
pub fn read(path: &Path) -> PayloadResult<String> {
...
pub fn read_bytes(path: &Path) -> PayloadResult<Vec<u8>> {
    fs::read(path).chain_err(|| format!("failed to read `{}`", path.display()))
}

pub fn write<P: AsRef<Path>, C: AsRef<[u8]>>(path: P, contents: C) -> PayloadResult<()> {
    let path = path.as_ref();
    fs::write(path, contents.as_ref()).chain_err(|| format!("failed to write `{}`", path.display()))
}

pub fn write_if_changed<P: AsRef<Path>, C: AsRef<[u8]>>(path: P, contents: C) -> PayloadResult<()> {
    (|| -> PayloadResult<()> {
...
pub fn append(path: &Path, contents: &[u8]) -> PayloadResult<()> {
    (|| -> PayloadResult<()> {
        let mut f = OpenOptions::new()
            .write(true)
            .append(true)
...
/// Creates a new file.
pub fn create<P: AsRef<Path>>(path: P) -> PayloadResult<File> {
    let path = path.as_ref();
    File::create(path).chain_err(|| format!("failed to create file `{}`", path.display()))
}

/// Opens an existing file.
pub fn open<P: AsRef<Path>>(path: P) -> PayloadResult<File> {
    let path = path.as_ref();
    File::open(path).chain_err(|| format!("failed to open file `{}`", path.display()))
}

pub fn mtime(path: &Path) -> PayloadResult<FileTime> {
...
pub fn mtime_recursive(path: &Path) -> PayloadResult<FileTime> {
    let meta = fs::metadata(path).chain_err(|| format!("failed to stat `{}`", path.display()))?;
    if !meta.is_dir() {
    ...
    let max_meta = walkdir::WalkDir::new(path)
        ...
        .filter_map(|e| match e {
            Ok(e) => Some(e),
            Err(e) => {
                // Ignore errors while walking. If Payload can't access it, the
            ...
            if e.path_is_symlink() {
                ...
                let sym_meta = match std::fs::symlink_metadata(e.path()) {
                    ...
                    Err(err) => {
                        ...
                        // race with unlinking?). Regardless, if Payload can't
                ...
                match e.metadata() {
                    ...
                    Err(err) => {
                        // Can't access the symlink target. If Payload can't
...
pub fn set_invocation_time(path: &Path) -> PayloadResult<FileTime> {
...
#[cfg(unix)]
pub fn path2bytes(path: &Path) -> PayloadResult<&[u8]> {
...
#[cfg(unix)]
pub fn bytes2path(bytes: &[u8]) -> PayloadResult<PathBuf> {
    use std::os::unix::prelude::*;
...
impl<'a> PathAncestors<'a> {
    fn new(path: &'a Path, stop_root_at: Option<&Path>) -> PathAncestors<'a> {
        let stop_at = env::var("__PAYLOAD_TEST_ROOT")
            .ok()
            .map(PathBuf::from)
            .or_else(|| stop_root_at.map(|p| p.to_path_buf()));
        PathAncestors {
            current: Some(path),
            //HACK: avoid reading `~/.payload/config` when testing Payload itself.
            stop_at,
        }
    }
}
...
pub fn create_dir_all(p: impl AsRef<Path>) -> PayloadResult<()> {
    _create_dir_all(p.as_ref())
}

fn _create_dir_all(p: &Path) -> PayloadResult<()> {
    fs::create_dir_all(p).chain_err(|| format!("failed to create directory `{}`", p.display()))?;
    Ok(())
}

pub fn remove_dir_all<P: AsRef<Path>>(p: P) -> PayloadResult<()> {
    _remove_dir_all(p.as_ref())
}

fn _remove_dir_all(p: &Path) -> PayloadResult<()> {
...
pub fn remove_dir<P: AsRef<Path>>(p: P) -> PayloadResult<()> {
    _remove_dir(p.as_ref())
}

fn _remove_dir(p: &Path) -> PayloadResult<()> {
    fs::remove_dir(p).chain_err(|| format!("failed to remove directory `{}`", p.display()))?;
    Ok(())
}

pub fn remove_file<P: AsRef<Path>>(p: P) -> PayloadResult<()> {
    _remove_file(p.as_ref())
}

fn _remove_file(p: &Path) -> PayloadResult<()> {
...
pub fn link_or_copy(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> PayloadResult<()> {
    let src = src.as_ref();
    let dst = dst.as_ref();
    _link_or_copy(src, dst)
}

fn _link_or_copy(src: &Path, dst: &Path) -> PayloadResult<()> {
    ...
    let link_result = if src.is_dir() {
        ...
        symlink(src, dst)
    } else if env::var_os("__PAYLOAD_COPY_DONT_LINK_DO_NOT_USE_THIS").is_some() {
        ...
        // `payload run` with SIGKILL. Copying seems to avoid the problem.
        // This shouldn't affect anyone except Payload's test suite because
        // it is very rare, and only seems to happen under heavy load and
        // rapidly creating lots of executables and running them.
        // See https://github.com/dustlang/payload/issues/7821 for the
        // gory details.
...
pub fn copy<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> PayloadResult<u64> {
...
/// supported on network filesystems. For the current uses in Payload, this is a
...
/// already exists.
pub fn create_dir_all_excluded_from_backups_atomic(p: impl AsRef<Path>) -> PayloadResult<()> {
    let path = p.as_ref();
    if path.is_dir() {
        return Ok(());
    }
    ...
    // we would risk a situation where payload is interrupted right after the directory
    ...
    // we can infer from it it's another payload process doing work.
...
# This file is a cache directory tag created by payload.
...
fn exclude_from_time_machine(path: &Path) {
    ...
    // doesn't prevent Payload from working
}
