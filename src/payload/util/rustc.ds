...
use crate::util::{self, profile, PayloadResult, PayloadResultExt, ProcessBuilder, StableHasher};
...
impl Rustc {
    ...
    pub fn new(
        ...
        let extract = |field: &str| -> PayloadResult<&str> {
            verbose_version
                .lines()
...
/// current toolchain.
///
/// https://github.com/dustlang/payload/issues/5315
/// https://github.com/dustlang/rust/issues/49761
#[derive(Debug)]
struct Cache {
    cache_location: Option<PathBuf>,
    dirty: bool,
    ...
    fn load(rustc: &Path, rustup_rustc: &Path, cache_location: Option<PathBuf>) -> Cache {
        match (cache_location, rustc_fingerprint(rustc, rustup_rustc)) {
            (Some(cache_location), Ok(rustc_fingerprint)) => {
                ...
                fn read(path: &Path) -> PayloadResult<CacheData> {
...
fn rustc_fingerprint(path: &Path, rustup_rustc: &Path) -> PayloadResult<u64> {
    ...
    // the rustup shim at `~/.payload/bin/rustup`), because `RUSTUP_TOOLCHAIN`
