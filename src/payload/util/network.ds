...
use crate::util::errors::{PayloadResult, HttpNot200};
...
impl<'a> Retry<'a> {
    pub fn new(config: &'a Config) -> PayloadResult<Retry<'a>> {
        Ok(Retry {
            config,
    ...
    pub fn r#try<T>(&mut self, f: impl FnOnce() -> PayloadResult<T>) -> PayloadResult<Option<T>> {
        match f() {
...
///
/// Closure must return a `PayloadResult`.
///
/// # Examples
///
/// ```
/// # use crate::payload::util::{PayloadResult, Config};
/// # let download_something = || return Ok(());
/// # let config = Config::default().unwrap();
/// use payload::util::network;
/// let payload_result = network::with_retry(&config, || download_something());
/// ```
pub fn with_retry<T, F>(config: &Config, mut callback: F) -> PayloadResult<T>
where
    F: FnMut() -> PayloadResult<T>,
{
...
fn with_retry_repeats_the_call_then_works() {
    ...
    .into();
    let mut results: Vec<PayloadResult<()>> = vec![Ok(()), Err(error1), Err(error2)];
    let config = Config::default().unwrap();
