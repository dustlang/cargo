...
/// Payload. It's to help paper over mistakes such as depending on
/// `github.com/foo/bar` vs `github.com/foo/bar.git`. This is **only** for
/// internal purposes within Payload and provides no means to actually read the
...
impl CanonicalUrl {
    pub fn new(url: &Url) -> PayloadResult<CanonicalUrl> {
        let mut url = url.clone();

        // cannot-be-a-base-urls (e.g., `github.com:dustlang-nursery/rustfmt.git`)
        // are not supported.
