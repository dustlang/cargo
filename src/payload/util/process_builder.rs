use crate::util::{process_error, read2, PayloadResult, PayloadResultExt};
use anyhow::bail;
...
impl ProcessBuilder {
    ...
    pub fn exec(&self) -> PayloadResult<()> {
    ...
    /// include our child process. If the child terminates then we'll reap them in Payload
    ...
    pub fn exec_replace(&self) -> PayloadResult<()> {
        imp::exec_replace(self)
    }

    /// Executes the process, returning the stdio output, or an error if non-zero exit status.
    pub fn exec_with_output(&self) -> PayloadResult<Output> {
        let mut command = self.build_command();
    ...
    ///
    /// ```rust
    /// use payload::util::{ProcessBuilder, process};
...
mod imp {
    ...
    pub fn exec_replace(process_builder: &ProcessBuilder) -> PayloadResult<()> {
