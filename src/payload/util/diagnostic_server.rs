...
//! cross-platform way for the `payload fix` command.
...
const DIAGNOSICS_SERVER_VAR: &str = "__PAYLOAD_FIX_DIAGNOSTICS_SERVER";
const PLEASE_REPORT_THIS_BUG: &str =
    "This likely indicates a bug in either rustc or payload itself,\n\
     ...
     https://github.com/dustlang/rust/issues\n\
...
impl<'a> DiagnosticPrinter<'a> {
    ...
    pub fn print(&mut self, msg: &Message) -> PayloadResult<()> {
        match msg {
            Message::Migrating {
                file,
                from_edition,
