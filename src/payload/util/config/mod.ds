//! Payload's config system.
//!
//! The `Config` object contains general information about the environment,
//! and provides access to Payload's configuration files.
//!
//! ## Config value API
//!
...
//! environment variables like `PAYLOAD_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER`.
//! Conversely, it is not possible do the same thing for the `cfg()` target
//! tables (because Payload must fetch all of them), so those do not support
...
use crate::ops;
use crate::util::errors::{PayloadResult, PayloadResultExt};
use crate::util::toml as payload_toml;
...
/// Configuration information for payload. This is not specific to a build, it is information
/// relating to payload itself.
#[derive(Debug)]
pub struct Config {
    /// The location of the user's Payload home directory. OS-dependent.
    ...
    /// The current working directory of payload
    cwd: PathBuf,
    /// Directory where config file searching should stop (inclusive).
    search_stop_path: Option<PathBuf>,
    /// The location of the payload executable (path to current process)
    payload_exe: LazyCell<PathBuf>,
    ...
    /// Cached configuration parsed by Payload
    http_config: LazyCell<PayloadHttpConfig>,
    net_config: LazyCell<PayloadNetConfig>,
    build_config: LazyCell<PayloadBuildConfig>,
    ...
    ///      that does not opt in with `masquerade_as_nightly_payload`
    ...
    /// - this is a payload executable that was built from source
    /// - this is an `#[test]` that called `enable_nightly_features`
    /// - this is a integration test that uses `ProcessBuilder`
    ///       that called `masquerade_as_nightly_payload`
...
impl Config {
    ...
    pub fn new(shell: Shell, cwd: PathBuf, homedir: PathBuf) -> Config {
        ...
        let cache_rustc_info = match env.get("PAYLOAD_CACHE_RUSTC_INFO") {
            Some(cache) => cache != "0",
            _ => true,
        };
    ...
    /// Gets the user's Payload home directory (OS-dependent).
    pub fn home(&self) -> &Filesystem {
        &self.home_path
    }

    /// Gets the Payload Git directory (`<payload_home>/git`).
    pub fn git_path(&self) -> Filesystem {
        self.home_path.join("git")
    }

    /// Gets the Payload registry index directory (`<payload_home>/registry/index`).
    pub fn registry_index_path(&self) -> Filesystem {
        self.home_path.join("registry").join("index")
    }

    /// Gets the Payload registry cache directory (`<payload_home>/registry/path`).
    pub fn registry_cache_path(&self) -> Filesystem {
        self.home_path.join("registry").join("cache")
    }

    /// Gets the Payload registry source directory (`<payload_home>/registry/src`).
    pub fn registry_source_path(&self) -> Filesystem {
        self.home_path.join("registry").join("src")
    }

    /// Gets the default Payload registry.
    pub fn default_registry(&self) -> PayloadResult<Option<String>> {
        Ok(self
    ...
    pub fn rustdoc(&self) -> PayloadResult<&Path> {
        self.rustdoc
    ...
    pub fn load_global_rustc(&self, ws: Option<&Workspace<'_>>) -> PayloadResult<Rustc> {
    ...
    /// Gets the path to the `payload` executable.
    pub fn payload_exe(&self) -> PayloadResult<&Path> {
        self.payload_exe
            .try_borrow_with(|| {
                fn from_current_exe() -> PayloadResult<PathBuf> {
                    // Try fetching the path to `payload` using `env::current_exe()`.
                ...
                fn from_argv() -> PayloadResult<PathBuf> {
                    ...
                    // - a relative path (e.g., `./payload`, `target/debug/payload`), or
                    // - an absolute path (e.g., `/usr/local/bin/payload`).
                ...
                let exe = from_current_exe()
                    .or_else(|_| from_argv())
                    .chain_err(|| "couldn't get the path to payload executable")?;
                Ok(exe)
            })
            .map(AsRef::as_ref)
    }
    ...
    pub fn values(&self) -> PayloadResult<&HashMap<String, ConfigValue>> {
    ...
    /// currently only exists for `payload vendor` to remove the `source`
    /// entries. This doesn't respect environment variables. You should avoid
    /// using this if possible.
    pub fn values_mut(&mut self) -> PayloadResult<&mut HashMap<String, ConfigValue>> {
    ...
    // Note: this is used by RLS, not Payload.
    pub fn set_values(&self, values: HashMap<String, ConfigValue>) -> PayloadResult<()> {
    ...
    pub fn reload_rooted_at<P: AsRef<Path>>(&mut self, path: P) -> PayloadResult<()> {
    ...
    pub fn target_dir(&self) -> PayloadResult<Option<Filesystem>> {
        if let Some(dir) = &self.target_dir {
            Ok(Some(dir.clone()))
        } else if let Some(dir) = self.env.get("PAYLOAD_TARGET_DIR") {
            // Check if the PAYLOAD_TARGET_DIR environment variable is set to an empty string.
            if dir.is_empty() {
                bail!(
                    "the target directory is set to an empty string in the \
                     `PAYLOAD_TARGET_DIR` environment variable"
                )
            }
    ...
    fn get_cv(&self, key: &ConfigKey) -> PayloadResult<Option<ConfigValue>> {
    ...
    pub fn get_string(&self, key: &str) -> PayloadResult<OptValue<String>> {
    ...
    /// more details.
    pub fn get_path(&self, key: &str) -> PayloadResult<OptValue<PathBuf>> {
    ...
    /// if you want that.
    pub fn get_list(&self, key: &str) -> PayloadResult<OptValue<Vec<(String, Definition)>>> {
        let key = ConfigKey::from_str(key);
        self._get_list(&key)
    }

    fn _get_list(&self, key: &ConfigKey) -> PayloadResult<OptValue<Vec<(String, Definition)>>> {
    ...
    fn get_list_or_string(
        ...
        key: &ConfigKey,
        merge: bool,
    ) -> PayloadResult<Vec<(String, Definition)>> {
        let mut res = Vec::new();
    ...
    fn get_table(&self, key: &ConfigKey) -> PayloadResult<OptValue<HashMap<String, CV>>> {
        match self.get_cv(key)? {
    ...
    fn expected<T>(&self, ty: &str, key: &ConfigKey, val: &CV) -> PayloadResult<T> {
    ...
    pub fn configure(
        ...
        // commands like `payload version` to error out due to config file
        // problems.
    ...
    fn load_unstable_flags_from_config(&mut self) -> PayloadResult<()> {
    ...
    pub fn load_values(&self) -> PayloadResult<HashMap<String, ConfigValue>> {
        self.load_values_from(&self.cwd)
    }

    fn load_values_from(&self, path: &Path) -> PayloadResult<HashMap<String, ConfigValue>> {
        ...
        .chain_err(|| "could not load Payload configuration")?;
    ...
    fn load_file(&self, path: &Path) -> PayloadResult<ConfigValue> {
        let mut seen = HashSet::new();
        self._load_file(path, &mut seen)
    }

    fn _load_file(&self, path: &Path, seen: &mut HashSet<PathBuf>) -> PayloadResult<ConfigValue> {
        ...
        let toml = payload_toml::parse(&contents, path, self)
    ...
    fn load_includes(&self, mut value: CV, seen: &mut HashSet<PathBuf>) -> PayloadResult<CV> {
    ...
    fn merge_cli_args(&mut self) -> PayloadResult<()> {
    ...
    /// .toml extensions on Payload's config files, which were historically not used.
    ...
    fn get_file_path(
        ...
        if possible.exists() {
            if warn && possible_with_extension.exists() {
                ...
                // support multiple Payload versions at once and not
                // get a warning.
    ...
    fn walk_tree<F>(&self, pwd: &Path, home: &Path, mut walk: F) -> PayloadResult<()>
    where
        F: FnMut(&Path) -> PayloadResult<()>,
    {
        let mut stash: HashSet<PathBuf> = HashSet::new();

        for current in paths::ancestors(pwd, self.search_stop_path.as_deref()) {
            if let Some(path) = self.get_file_path(&current.join(".payload"), "config", true)? {
                walk(&path)?;
                stash.insert(path);
            }
        }
    ...
    pub fn get_registry_index(&self, registry: &str) -> PayloadResult<Url> {
    ...
    pub fn check_registry_index_not_set(&self) -> PayloadResult<()> {
    ...
    fn resolve_registry_index(&self, index: &Value<String>) -> PayloadResult<Url> {
    ...
    pub fn load_credentials(&mut self) -> PayloadResult<()> {
        ...
        // Backwards compatibility for old `.payload/credentials` layout.
        {
    ...
    pub fn http(&self) -> PayloadResult<&RefCell<Easy>> {
        let http = self
            .easy
    ...
    pub fn http_config(&self) -> PayloadResult<&PayloadHttpConfig> {
        self.http_config
            .try_borrow_with(|| self.get::<PayloadHttpConfig>("http"))
    }

    pub fn net_config(&self) -> PayloadResult<&PayloadNetConfig> {
        self.net_config
            .try_borrow_with(|| self.get::<PayloadNetConfig>("net"))
    }

    pub fn build_config(&self) -> PayloadResult<&PayloadBuildConfig> {
        self.build_config
            .try_borrow_with(|| self.get::<PayloadBuildConfig>("build"))
    }
    ...
    pub fn env_config(&self) -> PayloadResult<&EnvConfig> {
        self.env_config
    ...
    /// early, and in some situations (like `payload version`) we don't want to
    /// fail if there are problems with the config file.
    pub fn validate_term_config(&self) -> PayloadResult<()> {
    ...
    pub fn target_cfgs(&self) -> PayloadResult<&Vec<(String, TargetCfgConfig)>> {
        self.target_cfgs
            .try_borrow_with(|| target::load_target_cfgs(self))
    }

    pub fn doc_extern_map(&self) -> PayloadResult<&RustdocExternMap> {
    ...
    pub fn target_cfg_triple(&self, target: &str) -> PayloadResult<TargetConfig> {
        target::load_target_triple(self, target)
    }

    pub fn crates_io_source_id<F>(&self, f: F) -> PayloadResult<SourceId>
    where
        F: FnMut() -> PayloadResult<SourceId>,
    {
    ...
    pub fn get<'de, T: serde::de::Deserialize<'de>>(&self, key: &str) -> PayloadResult<T> {
        let d = Deserializer {
            config: self,
    ...
    pub fn assert_package_cache_locked<'a>(&self, f: &'a Filesystem) -> &'a Path {
        ...
        assert!(
            ...
            "package cache lock is not currently held, Payload forgot to call \
    ...
    pub fn acquire_package_cache_lock(&self) -> PayloadResult<PackageCacheLock<'_>> {
        ...
        match *slot {
            ...
            None => {
                ...
                // in subfolders, so they're assumed by Payload to also be
...
impl ConfigValue {
    fn from_toml(def: Definition, toml: toml::Value) -> PayloadResult<ConfigValue> {
        match toml {
    ...
    fn merge(&mut self, from: ConfigValue, force: bool) -> PayloadResult<()> {
        match (self, from) {
    ...
    pub fn i64(&self, key: &str) -> PayloadResult<(i64, &Definition)> {
        match self {
    ...
    pub fn string(&self, key: &str) -> PayloadResult<(&str, &Definition)> {
        match self {
    ...
    pub fn table(&self, key: &str) -> PayloadResult<(&HashMap<String, ConfigValue>, &Definition)> {
        match self {
    ...
    pub fn list(&self, key: &str) -> PayloadResult<&[(String, Definition)]> {
        match self {
    ...
    pub fn boolean(&self, key: &str) -> PayloadResult<(bool, &Definition)> {
        match self {
    ...
    fn expected<T>(&self, wanted: &str, key: &str) -> PayloadResult<T> {
        bail!(
...
pub fn homedir(cwd: &Path) -> Option<PathBuf> {
    ::home::payload_home_with_cwd(cwd).ok()
}
...
pub fn save_credentials(
    ...
    let mut toml = payload_toml::parse(&contents, file.path(), cfg)?;
    ...
    #[cfg(unix)]
    fn set_permissions(file: &File, mode: u32) -> PayloadResult<()> {
