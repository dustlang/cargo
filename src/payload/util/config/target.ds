...
pub struct TargetConfig {
    /// Process to run as a wrapper for `payload run`, `test`, and `bench` commands.
    pub runner: OptValue<PathAndArgs>,
...
pub(super) fn load_target_cfgs(config: &Config) -> PayloadResult<Vec<(String, TargetCfgConfig)>> {
...
pub(super) fn load_target_triple(config: &Config, triple: &str) -> PayloadResult<TargetConfig> {
...
fn parse_links_overrides(
    ...
    config: &Config,
) -> PayloadResult<BTreeMap<String, BuildOutput>> {
