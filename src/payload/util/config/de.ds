...
pub(super) struct Deserializer<'config> {
    ...
    /// PAYLOAD_PROFILE_DEV_BUILD_OVERRIDE_ prefixes exist. But
    /// PAYLOAD_BUILD_TARGET should not check for prefixes because it would
    /// collide with PAYLOAD_BUILD_TARGET_DIR. See `ConfigMapAccess` for
    /// details.
...
impl<'de, 'config> de::MapAccess<'de> for ConfigMapAccess<'config> {
        ...
        V: de::DeserializeSeed<'de>,
    {
        ...
        // Example:
        //     PAYLOAD_BUILD_TARGET
        //     PAYLOAD_BUILD_TARGET_DIR
        // or
        //     PAYLOAD_PROFILE_DEV_DEBUG
        //     PAYLOAD_PROFILE_DEV_DEBUG_ASSERTIONS
        ...
        // starting with PAYLOAD_FOO_BAR_. This is a problem for keys like
        // PAYLOAD_BUILD_TARGET because checking for a prefix would incorrectly
        // match PAYLOAD_BUILD_TARGET_DIR. `deserialize_option` would have no
        // choice but to call `visit_some()` which would then fail if
        // PAYLOAD_BUILD_TARGET isn't set. So we check for these prefixes and
        // disallow them here.
    ...
    fn new(de: Deserializer<'config>) -> Result<ValueDeserializer<'config>, ConfigError> {
        ...
        let definition = {
            ...
            match (de.config.env.contains_key(env), de.config.get_cv(&de.key)?) {
                ...
                // PAYLOAD_FOO_BAR_* where `PAYLOAD_FOO_BAR` is not set.
                (_, None) => env_def,
            }
        };
