...
pub trait AppExt: Sized {
    ...
    fn arg_manifest_path(self) -> Self {
        self._arg(opt("manifest-path", "Path to Payload.toml").value_name("PATH"))
    }
...
pub fn multi_opt(
    ...
    // Note that all `.multiple(true)` arguments in Payload should specify
...
pub trait ArgMatchesExt {
    fn value_of_u32(&self, name: &str) -> PayloadResult<Option<u32>> {
    ...
    fn root_manifest(&self, config: &Config) -> PayloadResult<PathBuf> {
        if let Some(path) = self.value_of_path("manifest-path", config) {
            // In general, we try to avoid normalizing paths in Payload,
            // but in this particular case we need it to fix #3586.
            let path = paths::normalize_path(&path);
            if !path.ends_with("Payload.toml") {
                anyhow::bail!("the manifest-path must be a path to a Payload.toml file")
            }
            if !path.exists() {
                anyhow::bail!(
    ...
    fn workspace<'a>(&self, config: &'a Config) -> PayloadResult<Workspace<'a>> {
    ...
    fn jobs(&self) -> PayloadResult<Option<u32>> {
    ...
    fn packages_from_flags(&self) -> PayloadResult<Packages> {
        Packages::from_flags(
    ...
    fn compile_options(
        ...
        if let Some(ws) = workspace {
            ...
            // As for payload 0.50.0, this won't occur but if someone sneaks in
    ...
    fn new_options(&self, config: &Config) -> PayloadResult<NewOptions> {
    ...
    fn registry(&self, config: &Config) -> PayloadResult<Option<String>> {
        match self._value_of("registry") {
            Some(registry) => {
                ...
                if registry == CRATES_IO_REGISTRY {
                    ...
                    // as that will cause payload to use crates.io. This is required
    ...
    fn index(&self, config: &Config) -> PayloadResult<Option<String>> {
