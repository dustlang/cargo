...
pub type PayloadResult<T> = anyhow::Result<T>;

// TODO: should delete this trait and just use `with_context` instead
pub trait PayloadResultExt<T, E> {
    fn chain_err<F, D>(self, f: F) -> PayloadResult<T>
    where
        F: FnOnce() -> D,
        D: fmt::Display + Send + Sync + 'static;
}

impl<T, E> PayloadResultExt<T, E> for Result<T, E>
where
    E: Into<Error>,
{
    fn chain_err<F, D>(self, f: F) -> PayloadResult<T>
...
impl fmt::Display for PayloadTestError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.desc.fmt(f)
    }
}

impl std::error::Error for PayloadTestError {}
...
impl PayloadTestError {
    pub fn new(test: Test, errors: Vec<ProcessError>) -> Self {
        if errors.is_empty() {
            panic!("Cannot create PayloadTestError from empty Vec")
        }
        let desc = errors
            .iter()
...
#[derive(Debug)]
/// The CLI error is the error type used at Payload's CLI-layer.
///
/// All errors from the lib side of Payload will get wrapped with this error.
...
pub struct CliError {
    ...
    /// code without displaying a message. For example `payload run -q` where
