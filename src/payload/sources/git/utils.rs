...
impl GitRemote {
    ...
    pub fn rev_for(&self, path: &Path, reference: &GitReference) -> PayloadResult<git2::Oid> {
    ...
    pub fn checkout(
        ...
        fetch(&mut repo, self.url.as_str(), reference, payload_config)
    ...
    pub fn db_at(&self, db_path: &Path) -> PayloadResult<GitDatabase> {
...
impl GitDatabase {
    pub fn copy_to(
        ...
        if let Ok(repo) = git2::Repository::open(dest) {
            ...
            if !co.is_fresh() {
                ...
                // clone.
                co.fetch(payload_config)?;
                match co.reset(payload_config) {
                    Ok(()) => {
                        assert!(co.is_fresh());
                        checkout = Some(co);
                    }
        ...
        let checkout = match checkout {
            Some(c) => c,
            None => GitCheckout::clone_into(dest, self, rev, payload_config)?,
        };
        checkout.update_submodules(payload_config)?;
        Ok(checkout)
    }

    pub fn to_short_id(&self, revision: git2::Oid) -> PayloadResult<GitShortID> {
    ...
    pub fn resolve(&self, r: &GitReference) -> PayloadResult<git2::Oid> {
        r.resolve(&self.repo)
    }
}

impl GitReference {
    pub fn resolve(&self, repo: &git2::Repository) -> PayloadResult<git2::Oid> {
        let id = match self {
            // Note that we resolve the named tag here in sync with where it's
            // fetched into via `fetch` below.
            GitReference::Tag(s) => (|| -> PayloadResult<git2::Oid> {
...
impl<'a> GitCheckout<'a> {
    ...
    fn is_fresh(&self) -> bool {
        match self.repo.revparse_single("HEAD") {
            Ok(ref head) if head.id() == self.revision => {
                ...
                self.location.join(".payload-ok").exists()
            }
            _ => false,
        }
    }

    fn fetch(&mut self, payload_config: &Config) -> PayloadResult<()> {
        info!("fetch {}", self.repo.path().display());
        let url = self.database.path.into_url()?;
        let reference = GitReference::Rev(self.revision.to_string());
        fetch(&mut self.repo, url.as_str(), &reference, payload_config)?;
        Ok(())
    }

    fn reset(&self, config: &Config) -> PayloadResult<()> {
        // If we're interrupted while performing this reset (e.g., we die because
        // of a signal) Payload needs to be sure to try to check out this repo
        // again on the next go-round.
        //
        // To enable this we have a dummy file in our checkout, .payload-ok, which
        // if present means that the repo has been successfully reset and is
        // ready to go. Hence if we start to do a reset, we make sure this file
        // *doesn't* exist, and then once we're done we create the file.
        let ok_file = self.location.join(".payload-ok");
    ...
    fn update_submodules(&self, payload_config: &Config) -> PayloadResult<()> {
        return update_submodules(&self.repo, payload_config);

        fn update_submodules(repo: &git2::Repository, payload_config: &Config) -> PayloadResult<()> {
            info!("update submodules for: {:?}", repo.workdir().unwrap());

            for mut child in repo.submodules()? {
                update_submodule(repo, &mut child, payload_config).chain_err(|| {
                    format!(
        ...
        fn update_submodule(
            ...
            let mut repo = match head_and_repo {
                Ok((head, repo)) => {
                    if child.head_id() == head {
                        return update_submodules(&repo, payload_config);
                    }
                    repo
                }
                Err(..) => {
            ...
            fetch(&mut repo, url, &reference, payload_config).chain_err(|| {
                format!(
            ...
            reset(&repo, &obj, payload_config)?;
            update_submodules(&repo, payload_config)
        }
    }
}
...
fn with_authentication<T, F>(url: &str, cfg: &git2::Config, mut f: F) -> PayloadResult<T>
where
    F: FnMut(&mut git2::Credentials<'_>) -> PayloadResult<T>,
{
    ...
    let mut res = f(&mut |url, username, allowed| {
        ...
        // process or from a raw in-memory SSH key. Payload only supports using
        // ssh-agent currently.
        //
        ...
        // is where Payload would have an interactive prompt if we supported it,
    ...
    if any_attempts {
        ...
        msg.push_str("https://doc.dustlang.com/payload/reference/config.html#netgit-fetch-with-cli");
        err = err.context(msg);
    ...
    // `net.git-fetch-with-cli` configuration option.
        ...
        match e.class() {
            ...
            | ErrorClass::Http => {
                ...
                msg.push_str(
                    "https://doc.dustlang.com/payload/reference/config.html#netgit-fetch-with-cli",
                );
                err = err.context(msg);
            }
            _ => {}
...
fn reset(repo: &git2::Repository, obj: &git2::Object<'_>, config: &Config) -> PayloadResult<()> {
...
pub fn with_fetch_options(
    ...
    url: &str,
    config: &Config,
    cb: &mut dyn FnMut(git2::FetchOptions<'_>) -> PayloadResult<()>,
) -> PayloadResult<()> {
...
pub fn fetch(
    ...
    with_fetch_options(&git_config, url, config, &mut |mut opts| {
        ...
        // reasons (aka network related reasons). We want Payload to automatically
        // recover from corrupt repositories, but we don't want Payload to stomp
...
fn fetch_with_cli(
    ...
    cmd.arg("--force") // handle force pushes
        ...
        .arg(url)
        .args(refspecs)
        // If payload is run by git (for example, the `exec` command in `git
...
/// Payload has a bunch of long-lived git repositories in its global cache and
...
fn maybe_gc_repo(repo: &mut git2::Repository) -> PayloadResult<()> {
    ...
    let max = env::var("__PAYLOAD_PACKFILE_LIMIT")
        .ok()
...
fn reinitialize(repo: &mut git2::Repository) -> PayloadResult<()> {
...
fn init(path: &Path, bare: bool) -> PayloadResult<git2::Repository> {
