...
//! dependency graphs. It's infeasible for payload to download the entire contents
//! of the registry just to resolve one package's dependencies, for example. As
//! a result, payload needs some efficient method of querying what packages are
...
//! registry and Payload itself, so this route was not taken.
//!
//! Instead, Payload communicates with registries through a git repository
...
//!
//! Overall, the `$HOME/.payload` looks like this when talking about the registry:
//!
//! ```notrust
//! # A folder under which all registry metadata is hosted (similar to
//! # $HOME/.payload/git)
//! $HOME/.payload/registry/
//!
//!     # For each registry that payload knows about (keyed by hostname + hash)
//!     # there is a folder which is the checked out version of the index for
//!     # the registry in this location. Note that this is done so payload can
...
use crate::util::errors::PayloadResultExt;
use crate::util::hex;
use crate::util::interning::InternedString;
use crate::util::into_url::IntoUrl;
use crate::util::{restricted_names, PayloadResult, Config, Filesystem};

const PACKAGE_SOURCE_LOCK: &str = ".payload-ok";
pub const CRATES_IO_INDEX: &str = "https://github.com/dustlang/crates.io-index";
...
pub struct RegistrySource<'cfg> {
    source_id: SourceId,
    /// The path where crate files are extracted (`$PAYLOAD_HOME/registry/src/$REG-HASH`).
    src_path: Filesystem,
    ...
    /// as `Payload.lock` already exists and the index already contains the
    ...
    /// yanked.
    ///
    /// This is populated from the entries in `Payload.lock` to ensure that
    /// `payload update -p somepkg` won't unlock yanked entries in `Payload.lock`.
...
/// The maximum version of the `v` field in the index this version of payload
/// understands.
...
pub struct RegistryPackage<'a> {
    ...
    /// with a `Payload.lock` file.
    features2: Option<BTreeMap<InternedString, Vec<InternedString>>>,
    cksum: String,
    /// If `true`, Payload will skip this version when resolving.
    ///
    ...
    ///
    /// Added early 2018 (see <https://github.com/dustlang/payload/pull/4978>),
    ...
    /// and allow older versions of payload to ignore newer entries it doesn't
    /// understand. This is honored as of 1.51, so unfortunately older
    /// versions will ignore it, and potentially misinterpret version 2 and
    /// newer entries.
    ///
    /// The intent is that versions older than 1.51 will work with a
    /// pre-existing `Payload.lock`, but they may not correctly process `payload
    /// update` or build a lock from scratch. In that case, payload may
    /// incorrectly select a new package that uses a new index format. A
    /// workaround is to downgrade any packages that are incompatible with the
    /// `--precise` flag of `payload update`.
    v: Option<u32>,
}
...
impl<'a> RegistryDependency<'a> {
    /// Converts an encoded dependency in the registry to a payload dependency
    pub fn into_dep(self, default: SourceId) -> PayloadResult<Dependency> {
        ...
        // Unfortunately older versions of payload and/or the registry ended up
        ...
        // In Payload.toml, "registry" is None if it is from the default
...
pub trait RegistryData {
    ...
    fn download(&mut self, pkg: PackageId, checksum: &str) -> PayloadResult<MaybeLock>;
...
impl<'cfg> RegistrySource<'cfg> {
    ...
    pub fn config(&mut self) -> PayloadResult<Option<RegistryConfig>> {
        self.ops.config()
    }
    ...
    fn unpack_package(&self, pkg: PackageId, tarball: &File) -> PayloadResult<PathBuf> {
        // The `.payload-ok` file is used to track if the source is already
        // unpacked.
        ...
        for entry in tar.entries()? {
            ...
            // (or maliciously) overwrite source code from other crates. Payload
    ...
    fn do_update(&mut self) -> PayloadResult<()> {
    ...
    fn get_pkg(&mut self, package: PackageId, path: &File) -> PayloadResult<Package> {
        let path = self
...
impl<'cfg> Source for RegistrySource<'cfg> {
    fn query(&mut self, dep: &Dependency, f: &mut dyn FnMut(Summary)) -> PayloadResult<()> {
    ...
    fn fuzzy_query(&mut self, dep: &Dependency, f: &mut dyn FnMut(Summary)) -> PayloadResult<()> {
        self.index
    ...
    fn update(&mut self) -> PayloadResult<()> {
        ...
        // `Some("locked")` as other `Some` values indicate a `payload update
        // --precise` request
    ...
    fn download(&mut self, package: PackageId) -> PayloadResult<MaybePackage> {
    ...
    fn finish_download(&mut self, package: PackageId, data: Vec<u8>) -> PayloadResult<Package> {
        let hash = self.index.hash(package, &mut *self.ops)?;
        let file = self.ops.finish_download(package, hash, &data)?;
        self.get_pkg(package, &file)
    }

    fn fingerprint(&self, pkg: &Package) -> PayloadResult<String> {
    ...
    fn is_yanked(&mut self, pkg: PackageId) -> PayloadResult<bool> {
        if !self.updated {
            self.do_update()?;
        }
