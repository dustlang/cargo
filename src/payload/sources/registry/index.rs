...
//! path" as much as possible. Whenever you type `payload build` Payload will
//! *always* reparse the registry and learn about dependency information. This
//! is done because Payload needs to learn about the upstream crates.io crates
//! that you're using and ensure that the preexisting `Payload.lock` still matches
//! the current state of the world.
//!
//! Consequently, Payload "null builds" (the index that Payload adds to each build
...
//!
//! To solve all these issues a strategy is employed here where Payload basically
//! creates an index into the index. The first time a package is queried about
//! (first time being for an entire computer) Payload will load the contents
//! (slowly via libgit2) from the registry. It will then (slowly) parse every
//! single line to learn about its versions. Afterwards, however, Payload will
...
//! version extracted from each JSON blob. That way Payload can quickly and easily
...
//! Payload null build performance. It's expected that the improvements earned
...
//! implemented a null build of Payload itself would parse 3700 JSON blobs from
//! the registry and load 150 blobs from git. Afterwards it parses 150 JSON
//! blobs and loads 0 files git. Removing 200ms or more from Payload's startup
...
use crate::util::{internal, PayloadResult, Config, Filesystem, ToSemver};
use anyhow::bail;
use log::{debug, info};
...
/// Crates.io treats hyphen and underscores as interchangeable, but the index and old Payload do not.
/// Therefore, the index must store uncanonicalized version of the name so old Payload's can find it.
...
///
/// 1. Primarily Payload will parse the corresponding file for a crate in the
///    upstream crates.io registry. That's just a JSON blob per line which we
///    can parse, extract the version, and then store here.
///
/// 2. Alternatively, if Payload has previously run, we'll have a cached index of
///    dependencies for the upstream index. This is a file that Payload maintains
...
/// A representation of the cache on disk that Payload maintains of summaries.
/// Payload will initially parse all summaries in the registry and will then
...
impl<'cfg> RegistryIndex<'cfg> {
    ...
    pub fn hash(&mut self, pkg: PackageId, load: &mut dyn RegistryData) -> PayloadResult<&str> {
    ...
    /// though since this method is called quite a lot on null builds in Payload.
    pub fn summaries<'a, 'b>(
        &'a mut self,
        name: InternedString,
        req: &'b VersionReq,
        load: &mut dyn RegistryData,
    ) -> PayloadResult<impl Iterator<Item = &'a IndexSummary> + 'b>
    where
        'a: 'b,
    {
        ...
        // First up actually parse what summaries we have available. If Payload
        // has run previously this will parse a Payload-specific cache file rather
    ...
    pub fn query_inner(
            ...
            && self.query_inner_with_online(dep, load, yanked_whitelist, f, false)? != 0
        {
            ...
            // then payload will fail to download and an error message
    ...
    fn query_inner_with_online(
        ...
        let summaries = self
            ...
            // previously in `Payload.lock`
            .filter(|s| !s.yanked || yanked_whitelist.contains(&s.summary.package_id()))
            .map(|s| s.summary.clone());

        // Handle `payload update --precise` here. If specified, our own source
    ...
    pub fn is_yanked(&mut self, pkg: PackageId, load: &mut dyn RegistryData) -> PayloadResult<bool> {
        let req = VersionReq::exact(pkg.version());
        let found = self
            .summaries(pkg.name(), &req, load)?
...
impl Summaries {
    ...
    /// exists from a previous invocation of Payload (aka you're typing `payload
    ...
    pub fn parse(
        ...
        load: &mut dyn RegistryData,
        config: &Config,
    ) -> PayloadResult<Option<Summaries>> {
        ...
        let err = load.load(root, relative, &mut |contents| {
            ...
            for line in split(contents, b'\n') {
                ...
                // allow future payload implementations to break the
                // interpretation of each line here and older payload will simply
                // ignore the new lines.
                let summary = match IndexSummary::parse(config, line, source_id) {
                    Ok(summary) => summary,
                    Err(e) => {
                        // This should only happen when there is an index
                        // entry from a future version of payload that this
                        // version doesn't understand. Hopefully, those future
                        // versions of payload correctly set INDEX_V_MAX and
        ...
        // about to return, write that back out to disk so future Payload
    ...
    /// Payload.
    pub fn parse_cache(contents: Vec<u8>, last_index_update: &str) -> PayloadResult<Summaries> {
...
// +----------------+---+------------+---+
//
// The idea is that this is a very easy file for Payload to parse in future
...
// versions of Payload share the same cache they don't get too confused. The git
...
//   changes, different versions of payload won't get confused reading each
//   other's caches.

const CURRENT_CACHE_VERSION: u8 = 2;

impl<'a> SummariesCache<'a> {
    fn parse(data: &'a [u8], last_index_update: &str) -> PayloadResult<SummariesCache<'a>> {
        ...
        if *first_byte != CURRENT_CACHE_VERSION {
            bail!("looks like a different Payload's cache, bailing out");
        }
...
impl IndexSummary {
    ...
    fn parse(config: &Config, line: &[u8], source_id: SourceId) -> PayloadResult<IndexSummary> {
        // ****CAUTION**** Please be extremely careful with returning errors
        // from this function. Entries that error are not included in the
        // index cache, and can cause payload to get confused when switching
