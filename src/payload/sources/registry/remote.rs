...
pub struct RemoteRegistry<'cfg> {
    ...
    /// Path to the cache of `.crate` files (`$PAYLOAD_HOME/registry/path/$REG-HASH`).
...
impl<'cfg> RemoteRegistry<'cfg> {
    ...
    fn repo(&self) -> PayloadResult<&git2::Repository> {
        self.repo.try_borrow_with(|| {
            ...
            match git2::Repository::open(&path) {
                ...
                Err(_) => {
                    ...
                    // All versions of Payload, though, share the same PAYLOAD_HOME,
                    // so for compatibility with older Payload which *does* do
    ...
    fn head(&self) -> PayloadResult<git2::Oid> {
    ...
    fn tree(&self) -> PayloadResult<Ref<'_, git2::Tree<'_>>> {
        {
...
impl<'cfg> RegistryData for RemoteRegistry<'cfg> {
    ...
    fn update_index(&mut self) -> PayloadResult<()> {
        ...
        // is run multiple times, such as during `payload publish`.
        ...
        // problems with `.payload/config` configuration related to HTTP.
        //
    ...
    fn download(&mut self, pkg: PackageId, _checksum: &str) -> PayloadResult<MaybeLock> {
