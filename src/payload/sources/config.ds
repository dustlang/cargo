...
//! structure usable by Payload itself. Currently this is primarily used to map
//! sources to one another via the `replace-with` key in `.payload/config`.
...
struct SourceConfig {
    ...
    /// this configuration key was defined (such as the `.payload/config` path
    /// or the environment variable name).
    replace_with: Option<(String, String)>,
}

impl<'cfg> SourceConfigMap<'cfg> {
    pub fn new(config: &'cfg Config) -> PayloadResult<SourceConfigMap<'cfg>> {
    ...
    pub fn empty(config: &'cfg Config) -> PayloadResult<SourceConfigMap<'cfg>> {
    ...
    ) -> PayloadResult<Box<dyn Source + 'cfg>> {
        debug!("loading: {}", id);
...
",
    ...
    fn add(&mut self, name: &str, cfg: SourceConfig) -> PayloadResult<()> {
    ...
    fn add_config(&mut self, name: String, def: SourceConfigDef) -> PayloadResult<()> {
        ...
        fn url(val: &config::Value<String>, key: &str) -> PayloadResult<Url> {
