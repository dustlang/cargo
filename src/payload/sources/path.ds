...
use crate::ops;
use crate::util::{internal, paths, PayloadResult, PayloadResultExt, Config};
...
impl<'cfg> PathSource<'cfg> {
    /// Invoked with an absolute path to a directory that contains a `Payload.toml`.
    ///
    ...
    pub fn root_package(&mut self) -> PayloadResult<Package> {
    ...
    pub fn read_packages(&self) -> PayloadResult<Vec<Package>> {
        if self.updated {
            Ok(self.packages.clone())
        } else if self.recursive {
            ops::read_packages(&self.path, self.source_id, self.config)
        } else {
            let path = self.path.join("Payload.toml");
    ...
    pub fn list_files(&self, pkg: &Package) -> PayloadResult<Vec<PathBuf>> {
    ...
    fn _list_files(&self, pkg: &Package) -> PayloadResult<Vec<PathBuf>> {
        let root = pkg.root();
        ...
        let ignore_should_package = |relative_path: &Path, is_dir: bool| -> PayloadResult<bool> {
        ...
        let mut filter = |path: &Path, is_dir: bool| -> PayloadResult<bool> {
            let relative_path = path.strip_prefix(root)?;

            let rel = relative_path.as_os_str();
            if rel == "Payload.lock" {
                return Ok(pkg.include_lockfile());
            } else if rel == "Payload.toml" {
                return Ok(true);
            }
        ...
        // Attempt Git-prepopulate only if no `include` (see dustlang/payload#4135).
        if no_include_option {
            if let Some(result) = self.discover_git_and_list_files(pkg, root, &mut filter)? {
                return Ok(result);
            }
            // no include option and not git repo discovered (see dustlang/payload#7183).
            return self.list_files_walk_except_dot_files_and_dirs(pkg, &mut filter);
        }
        self.list_files_walk(pkg, &mut filter)
    }

    // Returns `Some(_)` if found sibling `Payload.toml` and `.git` directory;
    // otherwise, caller should fall back on full file list.
    fn discover_git_and_list_files(
        &self,
        pkg: &Package,
        root: &Path,
        filter: &mut dyn FnMut(&Path, bool) -> PayloadResult<bool>,
    ) -> PayloadResult<Option<Vec<PathBuf>>> {
        ...
        let manifest_path = repo_relative_path.join("Payload.toml");
        if index.get_path(&manifest_path, 0).is_some() {
            return Ok(Some(self.list_files_git(pkg, &repo, filter)?));
        }
        // Package Payload.toml is not in git, don't use git to guide our selection.
        Ok(None)
    }
    ...
    fn list_files_git(
        ...
        let untracked = statuses.iter().filter_map(|entry| match entry.status() {
            // Don't include Payload.lock if it is untracked. Packaging will
            // generate a new one as needed.
            git2::Status::WT_NEW if entry.path() != Some("Payload.lock") => {
        ...
        for (file_path, is_dir) in index_files.chain(untracked) {
            ...
            match file_path.file_name().and_then(|s| s.to_str()) {
                ...
                // our own `Payload.toml`, we keep going.
                Some("Payload.toml") => {
        ...
        #[cfg(unix)]
        fn join(path: &Path, data: &[u8]) -> PayloadResult<PathBuf> {
            use std::ffi::OsStr;
    ...
    fn list_files_walk_except_dot_files_and_dirs(
        ...
        let mut filter_ignore_dot_files_and_dirs =
            |path: &Path, is_dir: bool| -> PayloadResult<bool> {
                let relative_path = path.strip_prefix(root)?;
    ...
    fn walk(
        ...
        if !is_root && path.join("Payload.toml").exists() {
            return Ok(());
        }
        ...
        // See dustlang/payload#4268 and dustlang/payload#4270.
    ...
    pub fn last_modified_file(&self, pkg: &Package) -> PayloadResult<(FileTime, PathBuf)> {
        if !self.updated {
...
impl<'cfg> Source for PathSource<'cfg> {
    ...
    fn fuzzy_query(&mut self, _dep: &Dependency, f: &mut dyn FnMut(Summary)) -> PayloadResult<()> {
        for s in self.packages.iter().map(|p| p.summary()) {
            f(s.clone())
        }
        Ok(())
    ...
    fn download(&mut self, id: PackageId) -> PayloadResult<MaybePackage> {
    ...
    fn finish_download(&mut self, _id: PackageId, _data: Vec<u8>) -> PayloadResult<Package> {
    ...
    fn is_yanked(&mut self, _pkg: PackageId) -> PayloadResult<bool> {
        Ok(false)
    }
}
