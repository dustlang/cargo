...
impl<'cfg> Source for DirectorySource<'cfg> {
    ...
    fn update(&mut self) -> PayloadResult<()> {
        ...
        for entry in entries {
            ...
            // `Payload.toml` in them. This has the upside of being pretty
            ...
            let cksum_file = path.join(".payload-checksum.json");
            let cksum = paths::read(&path.join(cksum_file)).chain_err(|| {
                format!(
                    "failed to load checksum `.payload-checksum.json` \
                     of {} v{}",
                    pkg.package_id().name(),
            ...
            let cksum: Checksum = serde_json::from_str(&cksum).chain_err(|| {
                format!(
                    "failed to decode `.payload-checksum.json` of \
                     {} v{}",
                    pkg.package_id().name(),
                    pkg.package_id().version()
                )
    ...
    fn verify(&self, id: PackageId) -> PayloadResult<()> {
