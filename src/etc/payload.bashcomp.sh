...
command -v payload >/dev/null 2>&1 &&
_payload()
{
	...
	if [[ $cword -gt $dd_i ]]; then
		...
		if [[ "${cmd}" = @(test|bench) ]]; then
			...
			# Fallback to filename completion, useful with `payload run`.
			_filedir
		fi
		...
		if [[ "$cur" == -* ]]; then
			...
			COMPREPLY=( $( compgen -W "$__payload_commands" -- "$cur" ) )
		fi
	else
		case "${prev}" in
			--vcs)
...
__payload_commands=$(payload --list 2>/dev/null | awk 'NR>1 {print $1}')

_locate_manifest(){
	payload locate-project --message-format plain 2>/dev/null
}
