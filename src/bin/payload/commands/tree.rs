...
use payload::ops::tree::{self, EdgeKind};
use payload::ops::Packages;
use payload::util::print_available_packages;
use payload::util::PayloadResult;
...
pub fn cli() -> App {
    subcommand("tree")
        ...
        .arg(
            // Backwards compatibility with old payload-tree.
            Arg::with_name("version")
                .long("version")
                .short("V")
                .hidden(true),
        )
        .after_help("Run `payload help tree` for more detailed information.\n")
}
...
pub fn exec(config: &mut Config, args: &ArgMatches<'_>) -> CliResult {
    if args.is_present("version") {
        ...
        payload::drop_print!(config, "{}", version);
        return Ok(());
    }
    ...
    if args.is_present("all") {
        config.shell().warn(
            "The `payload tree` --all flag has been changed to --no-dedupe, \
    ...
    if args.is_present_with_zero_values("invert") {
        match &packages {
            Packages::Packages(ps) => {
                // Backwards compatibility with old syntax of `payload tree -i -p foo`.
...
fn parse_edge_kinds(config: &Config, args: &ArgMatches<'_>) -> PayloadResult<HashSet<EdgeKind>> {
