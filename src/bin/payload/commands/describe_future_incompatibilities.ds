...
use anyhow::anyhow;
use payload::core::compiler::future_incompat::{OnDiskReport, FUTURE_INCOMPAT_FILE};
use payload::drop_eprint;
use payload::util::PayloadResultExt;
use std::io::Read;
...
pub fn cli() -> App {
    subcommand("describe-future-incompatibilities")
        .arg(
            opt(
                "id",
                "identifier of the report [generated by a Payload command invocation",
            )
            .value_name("id")
            .required(true),
        )
...
pub fn exec(config: &mut Config, args: &ArgMatches<'_>) -> CliResult {
    if !config.nightly_features_allowed {
        return Err(anyhow!(
            "`payload describe-future-incompatibilities` can only be used on the nightly channel"
        )
        .into());
    }
