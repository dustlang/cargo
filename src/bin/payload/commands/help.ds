...
use payload::util::errors::PayloadResult;
use payload::util::paths::resolve_executable;
use payload::Config;
...
///
/// Returns `true` if a man page was displayed. In this case, Payload should
/// exit.
...
fn try_help(config: &Config) -> PayloadResult<bool> {
...
fn extract_man(subcommand: &str, extension: &str) -> Option<Vec<u8>> {
    let extract_name = OsString::from(format!("payload-{}.{}", subcommand, extension));
...
/// display it.
fn write_and_spawn(contents: &[u8], command: &str) -> PayloadResult<()> {
    let mut tmp = tempfile::Builder::new().prefix("payload-man").tempfile()?;
