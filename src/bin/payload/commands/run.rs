...
use payload::core::Verbosity;
use payload::ops::{self, CompileFilter, Packages};
...
pub fn cli() -> App {
    subcommand("run")
        ...
        .after_help("Run `payload help run` for more detailed information.\n")
}
    ...
    if let Packages::Packages(opt_in) = &compile_opts.spec {
        if let Some(pattern) = opt_in.iter().find(|s| is_glob_pattern(s)) {
            return Err(anyhow::anyhow!(
                "`payload run` does not support glob pattern `{}` on package selection",
                pattern,
            )
            .into());
        }
