    ...
    subcommand("vendor")
        ...
        .arg(
            Arg::with_name("tomls")
                .short("s")
                .long("sync")
                .help("Additional `Payload.toml` to sync and vendor")
                .value_name("TOML")
                .multiple(true),
        )
        .arg(
            Arg::with_name("respect-source-config")
                ...
                .help("Respect `[source]` config in `.payload/config`")
                .multiple(true),
        )
        .arg(
        ...
        .after_help("Run `payload help vendor` for more detailed information.\n")
}

pub fn exec(config: &mut Config, args: &ArgMatches<'_>) -> CliResult {
    // We're doing the vendoring operation ourselves, so we don't actually want
    // to respect any of the `source` configuration in Payload itself. That's
    // intended for other consumers of Payload, but we want to go straight to the
    // source, e.g. crates.io, to fetch crates.
    if !args.is_present("respect-source-config") {
        config.values_mut()?.remove("source");
    }

    // When we moved `payload vendor` into Payload itself we didn't stabilize a few
    ...
    let crates_io_payload_vendor_flag = if args.is_present("no-merge-sources") {
    ...
    if let Some(flag) = crates_io_payload_vendor_flag {
        return Err(anyhow::format_err!(
            "\
the crates.io `payload vendor` command has now been merged into Payload itself
and does not support the flag `{}` currently; to continue using the flag you
can execute `payload-vendor vendor ...`, and if you would like to see this flag
supported in Payload itself please feel free to file an issue at
https://github.com/dustlang/payload/issues/new
",
            flag
        )
        .into());
