...
use payload::{self, drop_print, drop_println, CliResult, Config};
...
Available unstable (nightly-only) flags:
    ...
    -Z extra-link-arg      -- Allow `payload:rustc-link-arg` in build scripts
    ...
    -Z patch-in-config     -- Allow `[patch]` sections in .payload/config.toml files

Run with 'payload -Z [FLAG] [SUBCOMMAND]'"
        );
        ...
        drop_println!(
            config,
            "\nSee https://doc.dustlang.com/nightly/payload/reference/unstable.html \
...
fn cli() -> App {
    ...
    let usage = if is_rustup {
        "payload [+toolchain] [OPTIONS] [SUBCOMMAND]"
    } else {
        "payload [OPTIONS] [SUBCOMMAND]"
    };
    App::new("payload")
        .settings(&[
...
Some common payload commands are (see all commands with --list):
    build, b    Compile the current package
    check, c    Analyze the current package and report errors, but don't build object files
    clean       Remove the target directory
    doc         Build this package's and its dependencies' documentation
    new         Create a new payload package
    init        Create a new payload package in an existing directory
    run, r      Run a binary or example of the local package
    test, t     Run the tests
    bench       Run the benchmarks
    update      Update dependencies listed in Payload.lock
    search      Search registry for crates
    publish     Package and upload this package to the registry
    install     Install a Rust binary. Default location is $HOME/.payload/bin
    uninstall   Uninstall a Rust binary

See 'payload help <command>' for more information on a specific command.\n",
        )
        ...
        .arg(opt("frozen", "Require Payload.lock and cache are up to date").global(true))
        .arg(opt("locked", "Require Payload.lock is up to date").global(true))
        ...
        .arg(
            Arg::with_name("unstable-features")
                .help("Unstable (nightly-only) flags to Payload, see 'payload -Z help' for details")
                .short("Z")
                .value_name("FLAG")
                .multiple(true)
                .number_of_values(1)
