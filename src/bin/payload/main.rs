...
use payload::core::shell::Shell;
use payload::util::{self, closest_msg, command_prelude, PayloadResult, CliResult, Config};
use payload::util::{CliError, ProcessError};
...
fn main() {
    ...
    pretty_env_logger::init_custom_env("PAYLOAD_LOG");
    #[cfg(not(feature = "pretty-env-logger"))]
    env_logger::init_from_env("PAYLOAD_LOG");

    let mut config = match Config::default() {
        Ok(cfg) => cfg,
        Err(e) => {
            let mut shell = Shell::new();
            payload::exit_with_error(e.into(), &mut shell)
        }
    };

    let result = match payload::ops::fix_maybe_exec_rustc(&config) {
        Ok(true) => Ok(()),
        Ok(false) => {
            let _token = payload::util::job::setup();
            cli::main(&mut config)
        }
    ...
    match result {
        Err(e) => payload::exit_with_error(e, &mut *config.shell()),
        Ok(()) => {}
    }
}

/// Table for defining the aliases which come builtin in `Payload`.
...
fn aliased_command(config: &Config, command: &str) -> PayloadResult<Option<Vec<String>>> {
...
fn execute_external_subcommand(config: &Config, cmd: &str, args: &[&str]) -> CliResult {
    let command_exe = format!("payload-{}{}", cmd, env::consts::EXE_SUFFIX);
    ...
    let payload_exe = config.payload_exe()?;
    let err = match util::process(&command)
        .env(payload::PAYLOAD_ENV, payload_exe)
        .args(args)
        .exec_replace()
    {
        Ok(()) => return Ok(()),
...
fn init_git_transports(config: &Config) {
    ...
    match payload::ops::needs_custom_http_transport(config) {
        Ok(true) => {}
        _ => return,
    }

    let handle = match payload::ops::http_handle(config) {
        Ok(handle) => handle,
        Err(..) => return,
    };
