...
//!
//! These tests are all disabled on dustlang/rust's CI, but run in Payload's CI.

use crate::{basic_manifest, main_file, project};
use payload::util::ProcessError;
...
pub fn disabled() -> bool {
    ...
    let run_cross_test = || -> PayloadResult<Output> {
        let p = project()
            .at("cross_test")
            .file("Payload.toml", &basic_manifest("cross_test", "1.0.0"))
