...
//!
//! See https://dustlang.github.io/payload/contrib/ for a guide on writing tests.
...
use payload::util::{is_ci, PayloadResult, ProcessBuilder, ProcessError, Rustc};
use serde_json::{self, Value};
use url::Url;

use self::paths::PayloadPathExt;
...
pub use payload_test_macro::payload_test;
...
impl ProjectBuilder {
    /// Root of the project, ex: `/path/to/payload/target/cit/t0/foo`
    pub fn root(&self) -> PathBuf {
        self.root.root()
    }

    /// Project's debug dir, ex: `/path/to/payload/target/cit/t0/foo/target/debug`
    ...
    pub fn build(mut self) -> Project {
        ...
        let manifest_path = self.root.root().join("Payload.toml");
        if !self.no_manifest && self.files.iter().all(|fb| fb.path != manifest_path) {
            self._file(Path::new("Payload.toml"), &basic_manifest("foo", "0.0.1"))
        }
        ...
        for file in self.files.iter() {
            ...
            if is_coarse_mtime() {
                ...
                // that if payload is called multiple times, the 2nd call will
                // see targets as "fresh". Without this, if payload finishes in
...
impl Project {
    ...
    /// Project's target dir, ex: `/path/to/payload/target/cit/t0/foo/target`
    ...
    /// File url for root, ex: `file:///path/to/payload/target/cit/t0/foo`
    pub fn url(&self) -> Url {
        path2url(self.root())
    }
    ...
    /// ex: `/path/to/payload/target/cit/t0/foo/target/debug/examples/libex.rlib`
    ...
    /// ex: `/path/to/payload/target/cit/t0/foo/target/debug/foo`
    ...
    /// ex: `/path/to/payload/target/cit/t0/foo/target/release/foo`
    ...
    /// ex: `/path/to/payload/target/cit/t0/foo/target/i686-apple-darwin/debug/foo`
    ...
    /// Creates a `ProcessBuilder` to run payload.
    /// Arguments can be separated by spaces.
    /// Example:
    ///     p.payload("build --bin foo").run();
    pub fn payload(&self, cmd: &str) -> Execs {
        let mut execs = self.process(&payload_exe());
    ...
    /// Safely run a process after `payload build`.
    ///
    ...
    ///
    /// If you have a test that needs to do `payload run` multiple
    /// times, you should instead use `payload build` and use this
    /// method to run the executable. Each time you call this,
    /// use a new name for `dst`.
    /// See dustlang/payload#5481.
    ...
    /// Returns the contents of `Payload.lock`.
    pub fn read_lockfile(&self) -> String {
        self.read_file("Payload.lock")
    }
    ...
    /// Modifies `Payload.toml` to remove all commented lines.
    pub fn uncomment_root_manifest(&self) {
        let contents = self.read_file("Payload.toml").replace("#", "");
        fs::write(self.root().join("Payload.toml"), contents).unwrap();
    }
...
// Path to payload executables
pub fn payload_dir() -> PathBuf {
    env::var_os("PAYLOAD_BIN_PATH")
        .map(PathBuf::from)
        .or_else(|| {
        ...
        .unwrap_or_else(|| panic!("PAYLOAD_BIN_PATH wasn't set. Cannot continue running test"))
}

pub fn payload_exe() -> PathBuf {
    payload_dir().join(format!("payload{}", env::consts::EXE_SUFFIX))
}
...
impl Execs {
    ...
    /// See `lines_match` for supported patterns.
    /// This is useful when checking the output of `payload build -v` since
    ...
    /// Typically used when testing payload commands that emit JSON.
    ...
    pub fn exec_with_output(&mut self) -> PayloadResult<Output> {
        self.ran = true;
        // TODO avoid unwrap
    ...
    pub fn masquerade_as_nightly_payload(&mut self) -> &mut Self {
        if let Some(ref mut p) = self.process_builder {
            p.masquerade_as_nightly_payload();
        }
        self
    }
    ...
    pub fn enable_mac_dsym(&mut self) -> &mut Self {
        if cfg!(target_os = "macos") {
            self.env("PAYLOAD_PROFILE_DEV_SPLIT_DEBUGINFO", "packed")
                .env("PAYLOAD_PROFILE_TEST_SPLIT_DEBUGINFO", "packed")
                .env("PAYLOAD_PROFILE_RELEASE_SPLIT_DEBUGINFO", "packed")
                .env("PAYLOAD_PROFILE_BENCH_SPLIT_DEBUGINFO", "packed");
        }
        self
    }
...
///   to match payload's "status" output and allows you to ignore the alignment.
...
/// (e.g., rustc) rather than Payload itself).
...
pub fn is_nightly() -> bool {
    env::var("PAYLOAD_TEST_DISABLE_NIGHTLY").is_err()
        && RUSTC
            .with(|r| r.verbose_version.contains("-nightly") || r.verbose_version.contains("-dev"))
}

pub fn process<T: AsRef<OsStr>>(t: T) -> payload::util::ProcessBuilder {
    _process(t.as_ref())
}

fn _process(t: &OsStr) -> payload::util::ProcessBuilder {
    let mut p = payload::util::process(t);

    // In general just clear out all payload-specific configuration already in the
    // environment. Our tests all assume a "default configuration" unless
    // specified otherwise.
    for (k, _v) in env::vars() {
        if k.starts_with("PAYLOAD_") {
            p.env_remove(&k);
        }
    }
    if env::var_os("RUSTUP_TOOLCHAIN").is_some() {
        ...
        let mut outer_payload = PathBuf::from(env::var_os("PAYLOAD").unwrap());
        outer_payload.pop();
        let new_path = env::join_paths(std::iter::once(outer_payload).chain(paths)).unwrap();
        p.env("PATH", new_path);
    }

    if cfg!(target_os = "macos") {
        // This makes the test suite run substantially faster.
        p.env("PAYLOAD_PROFILE_DEV_SPLIT_DEBUGINFO", "unpacked")
            .env("PAYLOAD_PROFILE_TEST_SPLIT_DEBUGINFO", "unpacked")
            .env("PAYLOAD_PROFILE_RELEASE_SPLIT_DEBUGINFO", "unpacked")
            .env("PAYLOAD_PROFILE_BENCH_SPLIT_DEBUGINFO", "unpacked");
    }

    p.cwd(&paths::root())
        .env("HOME", paths::home())
        .env("PAYLOAD_HOME", paths::home().join(".payload"))
        .env("__PAYLOAD_TEST_ROOT", paths::root())
        // Force Payload to think it's on the stable channel for all tests, this
        // should hopefully not surprise us as we add payload features over time and
        // payload rides the trains.
        .env("__PAYLOAD_TEST_CHANNEL_OVERRIDE_DO_NOT_USE_THIS", "stable")
        ...
        .env_remove("__PAYLOAD_DEFAULT_LIB_METADATA")
        .env_remove("RUSTC")
        .env_remove("RUSTDOC")
        ...
        .env_remove("EMAIL")
        .env_remove("USER") // not set on some dustlang docker images
        .env_remove("MFLAGS")
    ...
    if cfg!(target_os = "macos") {
        ...
        p.env("__PAYLOAD_COPY_DONT_LINK_DO_NOT_USE_THIS", "1");
    }
    p
}

pub trait ChannelChanger: Sized {
    fn masquerade_as_nightly_payload(&mut self) -> &mut Self;
}

impl ChannelChanger for payload::util::ProcessBuilder {
    fn masquerade_as_nightly_payload(&mut self) -> &mut Self {
        self.env("__PAYLOAD_TEST_CHANNEL_OVERRIDE_DO_NOT_USE_THIS", "nightly")
    }
}
...
pub fn payload_process(s: &str) -> Execs {
    let mut p = process(&payload_exe());
...
pub fn is_coarse_mtime() -> bool {
    ...
    // This should actually be a test that `$PAYLOAD_TARGET_DIR` is on an HFS
...
/// Some CI setups are much slower then the equipment used by Payload itself.
...
pub fn slow_cpu_multiplier(main: u64) -> Duration {
    lazy_static::lazy_static! {
        static ref SLOW_CPU_MULTIPLIER: u64 =
            env::var("PAYLOAD_TEST_SLOW_CPU_MULTIPLIER").ok().and_then(|m| m.parse().ok()).unwrap_or(1);
    }
