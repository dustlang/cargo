use crate::git::repo;
use crate::paths;
use payload::sources::CRATES_IO_INDEX;
use payload::util::Sha256;
...
/// Gets the path for local web API uploads. Payload will place the contents of a web API
...
pub struct RegistryBuilder {
    ...
    /// If `true`, configures `.payload/credentials` with some tokens.
    add_tokens: bool,
}
...
impl RegistryBuilder {
    ...
    /// Sets whether or not to initialize `.payload/credentials` with some tokens.
    /// Defaults to `true`.
...
/// `.payload/config` file to ensure that dependencies will use these packages
...
///
/// The `Payload.toml` file is automatically generated based on the methods
/// called on `Package` (for example, calling `dep()` will add to the
/// `[dependencies]` automatically). You may also specify a `Payload.toml` file
...
///     .build();
///
/// p.payload("run").with_stdout("24").run();
/// ```
#[must_use]
pub struct Package {
    name: String,
...
impl Package {
    ...
    /// causing Payload to skip over this version.
    ...
    pub fn payload_feature(&mut self, feature: &str) -> &mut Package {
        self.payload_features.push(feature.to_owned());
        self
    }

    /// Sets the index schema version for this package.
    ///
    /// See [`payload::sources::registry::RegistryPackage`] for more information.
    ...
    ///
    /// This does not actually use Payload's publishing system, but instead
    ...
    pub fn publish(&self) -> String {
        ...
        let deps = self
            ...
            .map(|dep| {
                ...
                // In Payload.toml, it is None if it is from crates.io.
    ...
    fn make_archive(&self) {
        ...
        if !self
            .files
            .iter()
            .any(|PackageFile { path, .. }| path == "Payload.toml")
        {
    ...
    fn append_manifest<W: Write>(&self, ar: &mut Builder<W>) {
        ...
        if !self.payload_features.is_empty() {
            manifest.push_str(&format!(
                "payload-features = {}\n\n",
                toml::to_string(&self.payload_features).unwrap()
            ));
        }
        ...
        self.append(ar, "Payload.toml", DEFAULT_MODE, &manifest);
    }
