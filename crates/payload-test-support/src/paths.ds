...
static PAYLOAD_INTEGRATION_TEST_DIR: &str = "cit";
...
lazy_static! {
    pub static ref GLOBAL_ROOT: PathBuf = {
        ...
        // If `payload test` is run manually then our path looks like
        // `target/debug/foo`, in which case our `path` is already pointing at
        // `target`. If, however, `payload test --target $target` is used then the
        ...
        path.push(PAYLOAD_INTEGRATION_TEST_DIR);
        path.mkdir_p();
        path
    };
...
// name.[*] Instead, we used the `payload-test-macro` crate to automatically
...
pub fn root() -> PathBuf {
    let id = TEST_ID.with(|n| {
        n.borrow().expect(
            "Tests must use the `#[payload_test]` attribute in \
...
pub fn echo_wrapper() -> std::path::PathBuf {
    let p = project()
        ...
        .file("Payload.toml", &basic_manifest("rustc-echo-wrapper", "1.0.0"))
        .file(
            "src/main.rs",
            r#"
            fn main() {
