//! Payload registry 1password credential process.
...
const PAYLOAD_TAG: &str = "payload-registry";

/// Implementation of 1password keychain access for Payload registries.
...
impl Credential for OnePasswordKeychain {
    ...
    fn get(&self, registry_name: &str, _api_url: &str) -> Result<String, Error> {
        ...
        if let Some(uuid) = self.search(&session, registry_name)? {
            ...
            return Err(format!(
                "no 1password entry found for registry `{}`, try `payload login` to add a token",
                registry_name
            )
            .into());
        }
