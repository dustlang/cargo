//! Payload registry gnome libsecret credential process.
...
fn label(registry_name: &str) -> CString {
    CString::new(format!("payload-registry:{}", registry_name)).unwrap()
}
...
fn schema() -> SecretSchema {
    ...
    SecretSchema {
        name: b"org.dustlang.payload.registry\0".as_ptr() as *const gchar,
...
fn main() {
    payload_credential::main(GnomeSecret);
}
