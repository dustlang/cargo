...
pub fn payload_test(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for token in item {
        ...
        let mut new_body =
            to_token_stream("let _test_guard = payload_test_support::paths::init_root();");
        ...
        // on CI.
        if build_std {
            let ts = to_token_stream("if !payload_test_support::is_nightly() { return }");
            new_body.extend(ts);
            let ts = to_token_stream(
                "if std::env::var(\"PAYLOAD_RUN_BUILD_STD_TESTS\").is_err() { return }",
            );
            new_body.extend(ts);
        }
